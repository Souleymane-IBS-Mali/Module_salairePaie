...
$titre = "Heures Supplémentaire de l'accord ".$nom_accord;
...
$head = paiementsalaireAccordHead($id_convention, $id_accord);
	...
	if($id_convention == 1){
		...
		if($num == 0){
			...
			for ($i=0; $i < count($taux); $i++) { 
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."heure_sup (fk_convention, taux, commentaire) VALUES (2,'".$taux[$i]."','".$description[$i]."')";
				$result = $db->query($sql);
			}
		}
		...
		if($num == 0){
			...
			for ($i=0; $i < count($taux); $i++) { 
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."heure_sup (fk_convention, taux, commentaire) VALUES (3,'".$taux[$i]."','".$description[$i]."')";
				$result = $db->query($sql);
			}
		}
		...
		if($num == 0){
			...
			$description = array("41è à 48è heure de la journée","48è + de la journé","jours feriés ou répos pendant la journé","jours feriés ou répos pendant la nuit");
			for ($i=0; $i < count($taux); $i++) { 
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."heure_sup (fk_convention, taux, commentaire) VALUES (4,'".$taux[$i]."','".$description[$i]."')";
				$result = $db->query($sql);
			}
		}
		...
		if($num == 0){
			...
			for ($i=0; $i < count($taux); $i++) { 
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."heure_sup (fk_convention, taux, commentaire) VALUES (5,'".$taux[$i]."','".$description[$i]."')";
				$result = $db->query($sql);
			}
		}
		...
		if($num == 0){
			...
			for ($i=0; $i < count($taux); $i++) { 
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."heure_sup (fk_convention, taux, commentaire) VALUES (7,'".$taux[$i]."','".$description[$i]."')";
				$result = $db->query($sql);
			}
		}
	}
	...
	if($action == "add_heure_sup"){
		...
		if(empty($message)){
			$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'heure_sup (taux, commentaire, fk_convention, fk_societe, fk_accord_etablissement) VALUES ("'.$taux.'","'.$desc.'",0,0,'.$id_accord.')';
			$result = $db->query($sql);
			if($result){
...
if($action == "create"){
	print '<form name="add_heure_supl"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_convention='.$id_convention.'&id_accord='.$id_accord.'">';
 ...
 print'</form>';
 print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=liste&id_convention='.$id_convention.'&id_accord='.$id_accord.'" class="button">Annuler</a></td></tr>';
 print '</table>';
}

if($action == "liste"){
	$id_convention = GETPOST("id_convention","int");
	print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer un nouveau type d'heure sup", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=create&id_convention='.$id_convention.'&id_accord='.$id_accord.'', '', 1), '', 0, 0, 0, 1);
 ...
 print '<td style="25%; color: darkblue; padding:20px" align="center"><label>Opération</label></tr>';
	...
	$heure_sup = "SELECT * FROM ".MAIN_DB_PREFIX."heure_sup WHERE fk_convention=".$id_convention." OR fk_accord_etablissement=".$id_accord;
	$result_heure_sup = $db->query($heure_sup);//= $db->query($covSql);
	if($result_heure_sup){
		$i = 0;
		$num = $db->num_rows($result_heure_sup);
		while ($i < $num){
			...
			print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?id_heure_sup='.$obj_heure_sup->rowid.'&action=edit_form&id_convention='.$id_convention.'&id_accord='.$id_accord.'">'.img_edit('Modifier','').'</a>';
			print '&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$_SERVER["PHP_SELF"].'?id_heure_sup='.$obj_heure_sup->rowid.'&action=supprimer&id_convention='.$id_convention.'&id_accord='.$id_accord.'">'.img_delete('Supprimer','').'&nbsp;&nbsp;&nbsp;</a>';
			print '</td>';
			print '</tr>';
 ...
 function myFunction(e){
	...
	if(!confirm('Cette suppression entraînera la suppression de :\\n toutes categories liées\\n Tous les echelons liés\\n Par conséquent les salaires de base liés')){
		var lien = '".$_SERVER["PHP_SELF"]."?mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=liste&id_convention=".$id_convention."&id_accord=".$id_accord."';
...
print load_fiche_titre($langs->trans("Modification d'un type"), '', '');
 print '<table><form action="'.$_SERVER["PHP_SELF"].'?id_heure_sup='.$id_heure_sup.'&id_convention='.$id_convention.'&id_accord='.$id_accord.'" method="post">';
 print '<input type="hidden" name="token" value="'.newToken().'">';
