...
$titre = "Indemnités de l'accord ".$nom_accord;
...
if(!empty(GETPOST("rowid_id", "int")))
	...
	if($action == "add_indemnite"){
		...
		if(empty($message)){
			...
			$sql_valeur = ' VALUES ("'.$libelle.'","'.$type_i.'","'.$desc_i.'",'.$applique.',"'.$exonere.'","'.$affiche_bulletin.'","'.$soumis_impot.'","'.$soumis_cotisation.'",0,'.$id_accord.',0,0';
			if($soumis_impot == "Oui"){
		...
		print '<form name="add_indemnite"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_accord='.$id_accord.'&id_convention='.$id_convention.'">';
	...
	print '<div id="container_degre_soumission_impot">
		<input type="number" name="degre_soumission_impot" min="1" max="100" value="'.(GETPOST("degre_soumission_impot", "09")?:"100").'" >%
	</div>';
	print '</td>';
	print '</tr>';	
	...
	print '</tr>';
		...
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle Indemnité", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=create&id_convention='.$id_convention.'&id_accord='.$id_accord , '', 1), '', 0, 0, 0, 1);
		...
		if($result){
			...
			while ($i < $num){
				...
				if($obj->active == 1)
					print '<tr class="pair"><td align="left" style="padding: 10px; width : 5%;"><a href="./detail_pr_ind.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_convention='.$id_convention.'&id_accord='.$id_accord.'&action=detailindemnite&id_indemnite='.$obj->rowid.'"><b>'.$obj->libelle.'<b></a></td>';
				else
		...
		$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."indemnite WHERE fk_accord_etablissement=".$id_accord." ORDER BY active DESC";
		$result = $db->query($indSql);//= $db->query($covSql);
		...
		if($result){
			...
			while ($i < $num2){
				...
				print '<td>';																			
				print'<a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_accord='.$id_accord.'&id_convention='.$id_convention.'&action='.$acts[$obj->active].'&id_indemnite='.$obj->rowid.'&token='.newToken().'">'.$actl[$obj->active].'</a>&nbsp;';
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_accord='.$id_accord.'&id_convention='.$id_convention.'&action=detail&id_indemnite='.$obj->rowid.'">'.img_edit().'</a>';	
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_accord='.$id_accord.'&id_convention='.$id_convention.'&action=supprimer&id_indemnite='.$obj->rowid.'">'.img_delete().'</a>&nbsp;';
				print "</td></tr>";
				$i ++;
			}
			//$db->free($result);
		...
		if($action == "saveedit_indemnite"){
			...
			if($action == 'detail'){
				...
				$id = $id_indemnite;
				$head = Prime_indm_accord_Head($id, 1, $id_convention, $id_accord);
				...
				print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_accord='.$id_accord.'&id_indemnite='.$obj->rowid.'&action=edit_indemnite">Modifier</a></td></tr>';
				print "</table>";
		}
		...
		if($action == 'edit_indemnite'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_indemnite='.$obj->rowid.'&id_accord='.$id_accord.'&id_convention='.$id_convention.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=detail&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'edit_bareme'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_indemnite='.$id_indemnite.'&rowid_id='.$rowid.'&id_convention='.$id_convention.'&id_accord='.$id_accord.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&action=liste_condition&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'nouvelle_condition_form'){
			...
			$verif_obj = $db->fetch_object($verif_result);	
					print '<div >';
					print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=accord_etablissement&id_indemnite='.$id_indemnite.'&id_convention='.$id_convention.'&id_accord='.$id_accord.'">';
					print '<input type="hidden" name="token" value="'.newToken().'">';
		...
		if($action == "liste_condition"){
			...
			//------------------------------------------------------------------------------------------------------------------------------------------
				...
				//------------------------------------------------------------------------------------------------------------------------------------------
						...
						print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'&id_accord='.$id_accord.'&id_convention'.$id_convention.'">'.img_delete('', '').'&nbsp;</a>';								
						print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&id_accord='.$id_accord.'&id_convention'.$id_convention.'&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'">'.img_edit('', '').'</a></td>';								
						print "<tr>";
						$numero ++;
						$i ++;
				}
				//$db->free($result_ind);
				if($num == 0) print "<tr><td align='center' colspan='8'>Aucun barème disponible cette prime\indemnité.<br>
