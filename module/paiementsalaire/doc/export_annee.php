...
$mois_tab = array(" Janvier "," Février "," Mars "," Avril "," Mai "," Juin "," Juillet "," Août "," Septembre "," Octobre "," Novembre "," Décembre ", " 13è Mois ");
      ...
      $soc_sql .= " LEFT JOIN ".MAIN_DB_PREFIX."salairepaie_societe as salp on soc.rowid = salp.fk_societe WHERE soc.rowid=".$db->escape($id_societe);
...
$bulletin_sql = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee." AND fk_societe=".$db->escape($id_societe);
      ...
      $res_bulletin = $db->query($bulletin_sql);
        if($res_bulletin)
            ...
            //Matricule
                  ...
                  $bulletin_sql_anc .= " WHERE annee=".$db->escape($annee)." AND fk_societe=".$db->escape($id_societe);
                  ...
                  $bulletin_sql .= " WHERE annee=".$db->escape($annee)." AND fk_societe=".$db->escape($id_societe);
                  $res_bulletin = $db->query($bulletin_sql);
                  if($res_bulletin){
                        $i = 0;
                  ...
                  $bulletin_sql_exc .= " WHERE annee=".$db->escape($annee)." AND fk_societe=".$db->escape($id_societe);
                  ...
                  $bulletin_sql_hs .= " WHERE annee=".$db->escape($annee)." AND fk_societe=".$db->escape($id_societe);
                  ...
                  $bulletin_sql_av .= " WHERE annee=".$db->escape($annee)." AND fk_societe=".$db->escape($id_societe);
...
//--------------------------------------------------------------------
            ...
            $tout_sal_sql .= " WHERE annee=".$db->escape($annee)." AND fk_societe = ".$db->escape($id_societe);
            ...
            $res_tout_sal = $db->query($tout_sal_sql);
                  ...
                  if($res_tout_sal){
                        ...
                        while ($obj_tout_sal = $db->fetch_object($res_tout_sal)){
                              ...
                              $bulletin_sql = "SELECT rowid, salaire_base, sursalaire, salaire_brut, salaire_brut_imposable, salaire_brut_cotisable, net_payer FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$db->escape($annee)." AND fk_salarie = ".$obj_tout_sal->fk_salarie;
                              $res_bulletin = $db->query($bulletin_sql);
                              $num_all = $db->num_rows($res_bulletin);
                              $cout_annee = 0;
                              ...
                              $un_sal_sql .= " WHERE annee=".$db->escape($annee)." AND fk_salarie = ".$obj_tout_sal->fk_salarie;
                              ...
                              //Objet Utilisateur
                                    ...
                                    $nb_jours = 0;
                                    for ($i=1; $i < 14; $i++) { 
                                          $salSql = "SELECT nb_jour_travailler FROM ".MAIN_DB_PREFIX."bulletin where mois=".$i." AND annee=".$db->escape($annee)." AND fk_salarie=".$obj_tout_sal->fk_salarie;
                              ...
                              for ($j=0; $j < count($array_id_hs); $j++) {
                                    ...
                                    if($affiche_reg_its == 1){
                                          ...
                                          $sql_reg .= " WHERE mois = 12 AND fk_salarie = ".$db->escape($obj_tout_sal->fk_salarie)." AND annee = ".$db->escape($annee);
...
$action_effectue = "Exportation des états de salaire de la société ".$nom_soc." de l'année ".$db->escape($annee)." dans société Salaire";
...
$filename = $nom_soc."_".$annee.'_'.gmdate('D_d_M_Y_H:i:s'); //le nom du fichier
