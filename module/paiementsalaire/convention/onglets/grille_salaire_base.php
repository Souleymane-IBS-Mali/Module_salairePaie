...
if($result){
	...
	$nom_convention = "<b><mark>".$obj_conv->nom."</mark></b>";
}
$message = "";
...
$indice = 0;
		...
		while ($i < $num){
			...
			if($numEch > 0){
				...
				while ($j < $numEch){
					...
					if($j==0){
						if(empty(GETPOST($nom_sal_base)))
							$message .= 'Le champ "'.$obj->code_categorie.'==>'.$obj_ech->libelle.'" est Obligatoire<br>';
						else {
							$salaire_base[count($salaire_base)] = GETPOST($nom_sal_base);
	...
	if ($res2){
		$action = "afficher";
		$message = "Une grille avec Categorie, Echelon et Salaire de Base Enregistrer avec succès";
	}else {
		$action = "create";
	...
	if($action == "afficher"){
		$titre = "Grille de salaire de la convention ".$nom_convention;
	...
	print dol_get_fiche_head($head, 'grille', "", -1, '');
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une grille", '', 'fa fa-plus-circle', $_SERVER['PHP_SELF'].'?action=create&id_convention='.$id_convention.'' , '', 1), '', 0, 0, 0, 1);
				...
				while ($i < $num){
					...
					if($num > 1){
						if($obj_grille->active == 1){
							...
							print'<td><a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&action='.$acts[$obj_grille->active].'&id_convention='.$id_convention.'&id_grille='.$obj_grille->rowid.'&token='.newToken().'">'.$actl[$obj_grille->active].'</a></td>';
						}
					}else{
						print '<tr class="pair"><td class="pair" align="left" style="padding: 10px; width : 5%;"><b>'.$obj_grille->nom.'<b></td><td >'.$obj_grille->commentaire.'</td>';
						print'<td><a class="reposition" href="#">'.$actl[$obj_grille->active].'</a></td>';
...
$covSql = "SELECT rowid, code_categorie FROM ".MAIN_DB_PREFIX."dcategories WHERE fk_convention=".$id_convention;
	...
	if($action == 'create'){	
		...
		<td style="padding-top: 10px" id="nom" ><input id="nom_grille" name ="nom_grille" Value="'.(GETPOST("nom_grille")?:"").'" size="30" type="text" /></td>';
		print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Description</label></td>
		<td style="padding-top: 10px" id="nom" ><textarea id="commentaire" name ="commentaire">'.(GETPOST("commentaire")?:"Avec la date de création").'</textarea></td>';
		print "</tr>";
		...
		while ($i < $num){
			...
			if($numEch > 0){
				...
				while ($j < $numEch){
					...
					if($j==0){
						print '<td style="width : 5%">'.($obj_ech->libelle ? : "N/A");									
						print '</td><td><input type="text" name="'.$nom_sal_base.'" value="'.(GETPOST($nom_sal_base)?:"").'" > </td></tr>';
					}else{
						print '<tr class="pair"><td style="width : 5%">'.($obj_ech->libelle ? : "N/A");									
						print '</td><td><input type="text" name="'.$nom_sal_base.'" value="'.(GETPOST($nom_sal_base)?:"").'" > </td></tr>';
					}
