	...
	if($action == "generer"){
				...
				if($result_global){
					...
					if($num_sal_exist > $nombre_salarie_licence){
						...
						//On supprime les lignes existantes et on insert une ligne pour pouvoir cloturé le bulletin
						suppression_salaire_mois($mois, $annee, $id_societe);
						$sql_bulletin = 'Insert into '.MAIN_DB_PREFIX.'bulletin (nom, prenom, fk_salarie, annee, mois, fk_societe)
							VALUES("aucun", "aucun", 0,'.$annee.','.$mois.','.$id_societe.')';
						...
						if($res_bulletin){
							...
							$action_effectue = "Génération avec succès des bulletins de ".$mois_tab[$mois-1]." ".$annee." de la société ".$obj_soc->nom." de ".count($tab_sal_rowid)." salarié";
							$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
							$sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Generation Bulletins")';
							$db->query($sql_log);
							$message = "Les salaires des Employés du mois".$mois_tab[$mois-1]." sont générés avec succès";
...
function suppression_salaire_mois($mois, $annee, $id_societe){
	global $db;
