...
$action= GETPOST("action") ? : "afficher";
...
if($action == "add"){
	...
	if(empty($message)){
		$code = GETPOST('code_categorie');
		$nom = GETPOST('nom_categorie');
...
if($action == "afficher"){
$titre = "Catégories de la convention ".$nom_convention;
...
print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Ajouter une Catégorie", '', 'fa fa-plus-circle', $_SERVER['PHP_SELF'].'?leftmenu=convention&action=create&id_convention='.$id_convention.'' , '', 1), '', 0, 0, 0, 1);
...
$covSql = "SELECT rowid, code_categorie FROM ".MAIN_DB_PREFIX."dcategories WHERE fk_convention=".$id_convention;
	...
	if(!empty($result)){
		...
		while ($i < $num){
			...
			if($numEch > 0){
				print '<tr class="pair"><td align="left" rowspan="'.($numEch ? : 1).'" style="padding: 3px; width : 5%;"><a href="./detail.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=detailcategorie&id_categ='.$obj->rowid.'&id_convention='.$id_convention.'"><b>'.$obj->code_categorie.'<b></a></td>';
				$j=0;
				while ($j < $numEch){
				...
				print '<tr class="pair"><td align="left" style="padding: 3px; width : 5%;"><a href="./detail.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=detailcategorie&id_categ='.$obj->rowid.'&id_convention='.$id_convention.'"><b>'.$obj->code_categorie.'<b></A></td>';
...
if($action == 'create'){	
	print load_fiche_titre($langs->trans("Ajout d'une catégorie à la convention ".$nom_convention), '', '');
$head = paiementsalaireConventionHead($id_convention);
print dol_get_fiche_head($head, 'categorie', "", -1, '');		
	//Titre 
	...
	<td style="padding-top: 10px" id="nom" ><input id="code_categorie" name ="code_categorie" size="30" type="text" value="'.(GETPOST("code_categorie")?:"").'" /></td>';
	print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Nom Catégorie</label></td>
	<td style="padding-top: 10px" id="nom" ><input id="nom_categorie" name ="nom_categorie" value="'.(GETPOST("nom_categorie")?:"").'" size="30" type="text" /></td>';
	print "</tr>";
