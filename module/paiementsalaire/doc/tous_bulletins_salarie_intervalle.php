...
// Génération par intervalle : mois + année
// Paramètres attendus dans l'URL :
// mois_debut=1&annee_debut=2026&mois_fin=6&annee_fin=2026
// Compatibilité : si l'intervalle n'est pas fourni, on garde l'ancien comportement avec annee=YYYY.
...
// Valeurs par défaut pour éviter les warnings/fatal errors lors de la génération PDF
$info_soc = new stdClass();
$info_soc->societe_mere = 0;
$info_soc->afficher_regularisation_its = 0;
$cachet_statut = new stdClass();
$cachet_statut->avant_cloture = 0;
$cachet_statut->apres_cloture = 0;
$filePath = "";
$id_salarie = 0;

if(empty($annee_debut)) $annee_debut = $annee;
if(empty($annee_fin)) $annee_fin = $annee;
if(empty($mois_debut)) $mois_debut = 1;
if(empty($mois_fin)) $mois_fin = 13;
...
$date_debut_key = ($annee_debut * 100) + $mois_debut;
$date_fin_key = ($annee_fin * 100) + $mois_fin;

// Si l'utilisateur inverse les dates, on remet automatiquement dans le bon ordre.
if($date_debut_key > $date_fin_key){
  $tmp_key = $date_debut_key;
  $date_debut_key = $date_fin_key;
  $date_fin_key = $tmp_key;

  $tmp_mois = $mois_debut;
  $tmp_annee = $annee_debut;
  $mois_debut = $mois_fin;
...
if($id_societe){
  ...
  $bulletin_sql = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE fk_salarie=".$fk_salarie;
  $bulletin_sql .= " AND ((annee * 100) + mois) BETWEEN ".$date_debut_key." AND ".$date_fin_key;
  $bulletin_sql .= " ORDER BY annee ASC, mois ASC";
        ...
        if($res_bulletin){
          ...
          if ($num_all > 0){
            ...
            if($res_sal){
              ...
              if(!$obj_salarie){
                die("Salarié introuvable");
              }

                $user_Sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."user where rowid=".(int) $obj_salarie->fk_user;
                $user_Result = $db->query($user_Sql);
                if($user_Result){
                  $obj_user_tmp = $db->fetch_object($user_Result);
                  if($obj_user_tmp){
                    $id_salarie = (int) $obj_user_tmp->rowid;
                  }
                }
                ...
                if($result_soc){
                  $tmp_info_soc = $db->fetch_object($result_soc);
                  if($tmp_info_soc){
                    $info_soc = $tmp_info_soc;
                  }
                }

                  $sql_select = "SELECT avant_cloture, apres_cloture FROM ".MAIN_DB_PREFIX."statut_cachet WHERE fk_societe=".$id_societe;
                  $res_cachet = $db->query($sql_select);
                  if($res_cachet){
                    $tmp_cachet = $db->fetch_object($res_cachet);
                    if($tmp_cachet){
                      $cachet_statut = $tmp_cachet;
                    }
                  }
...
// Création de la class PDF
  class PDF extends FPDF {
   ...
   //--------------------------------------------------------------------------------------------
    ...
    $nb_jours = 0;
    if($result){
      $obj_nb_jours = $db->fetch_object($result);
      if($obj_nb_jours){
        $nb_jours = (float) $obj_nb_jours->jour;
      }
    }
    ...
    $pdf->Cell(35,4, utf8_decode("Salaire de base normale"),0,0,'L');
      ...
      $obj_bulletin_anc = $res_bulletin_anc ? $db->fetch_object($res_bulletin_anc) : null;
      if(!$obj_bulletin_anc){
        $obj_bulletin_anc = new stdClass();
        $obj_bulletin_anc->libelle = "Ancienneté";
        $obj_bulletin_anc->taux = 0;
      }
     ...
     $pdf->SetFont('Helvetica','',9);
      ...
      if($bulletin_prest_res){
        ...
        while ($j < $num){
          ...
          if($prestation && !in_array($prestation->fk_organisme, $id_organisme))
            if($obj_bulletin_prest && $obj_bulletin_prest->affiche_bulletin == "Oui"){
              if($j == 0){
                if($avance)
                  $y = $pdf->GetY() + 2;
                else $y = $pdf->GetY() + 6;
        ...
        while ($j < $num){
          $obj_bulletin_taxe = $db->fetch_object($bulletin_taxe_res);
            if($obj_bulletin_taxe->affiche_bulletin == "Oui"){
              ...
              if($mois == 12){
                $reg = 0;
                $article29_obj = new stdClass();
                $article29_obj->difference = 0;
    ...
    //à gauche
      ...
      $pdf->SetY($y+4.5);
      $pdf->MultiCell(59,17, "",1,'');
      if(!empty($filePath) && is_readable($filePath) && $obj_bulletin->cloture == "oui" && $cachet_statut->apres_cloture == 1){
        $pdf->Image($filePath,150,$y+5, 40,19);
      }elseif(!empty($filePath) && is_readable($filePath) && $obj_bulletin->cloture == "non" && $cachet_statut->avant_cloture == 1){
        $pdf->Image($filePath,150,$y+5, 40,19);
      }

  $i ++;
          }
          $doc = $obj_bulletin->nom.'_'.$obj_bulletin->prenom.'_du_'.$mois_debut.'_'.$annee_debut.'_au_'.$mois_fin.'_'.$annee_fin;
          $pdf->Output($doc, $mode);
  }
}
}
...
function apres_virgule($db, $id_societe, $valeur, $decalage){
  ...
  function pdf_pagehead(&$pdf, $onglet_salarie){
	...
	$bulletin_sql = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE fk_salarie=".$fk_salarie." AND annee='".$annee."' AND mois='".$mois."'";
		...
		$bulletin_obj = $rest_bulletin ? $db->fetch_object($rest_bulletin) : null;
		if(!$bulletin_obj){
			return;
		}
		...
		if(!empty($info_soc) && $info_soc->societe_mere == 0){
...
*/
  ...
  //(!empty($conf->global->MAIN_INFO_SOCIETE_GENCODE) ? $conf->global->MAIN_INFO_SOCIETE_GENCODE : '')))
    ...
    if(!empty($info_soc) && $info_soc->societe_mere == 0){
      $societe_Sql = "SELECT * FROM ".MAIN_DB_PREFIX."societe where rowid=".$id_societe;
      $societe_Result = $db->query($societe_Sql);
      $societe_obj = $societe_Result ? $db->fetch_object($societe_Result) : null;
      if($societe_obj){
