...
$titre = "Primes de la convention ".$nom_convention;
...
//print '<hr>';
	...
	if($action == "add_prime"){
		...
		print '<form name="add_prime"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'">';
		...
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle Prime", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=create&id_convention='.$id_convention , '', 1), '', 0, 0, 0, 1);
		...
		$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."primes WHERE fk_convention=".$id_convention." ORDER BY active DESC";
		...
		if($result){
			...
			while ($i < $num){
				...
				if($obj->active == 1)
					print '<tr class="pair"><td align="left" style="padding: 10px; width : 5%;"><a href="./detail_pr_ind.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=detailprime&id_prime='.$obj->rowid.'"><b>'.$obj->libelle.'<b></a></td>';
				else
				...
				print '<td>';																			
				print'<a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action='.$acts[$obj->active].'&id_prime='.$obj->rowid.'&token='.newToken().'">'.$actl[$obj->active].'</a>&nbsp;';
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=detail&id_prime='.$obj->rowid.'">'.img_edit().'</a>';	
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=supprimer&id_prime='.$obj->rowid.'">'.img_delete().'</a>&nbsp;';
				print "</td></tr>";
				$i ++;
			}
			if($num == 0) print "<tr><td align='center' colspan='4'>Auccune Prime</td></tr>";
			$db->free($result);
			print "<script>
		...
		if($action == "saveedit_prime"){
			...
			if($action == 'detail'){
				...
				$id = $id_prime;
				$head = Prime_indm_convention_Head($id, 2, $id_convention);
				...
				print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&id_prime='.$obj->rowid.'&action=edit_prime">Modifier</a></td></tr>';
				print "</table>";
		}
		...
		if($action == 'edit_prime'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_prime='.$obj->rowid.'&id_convention='.$id_convention.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&action=detail&id_convention='.$id_convention.'&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'edit_bareme'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_prime='.$id_prime.'&rowid_id='.$rowid.'&id_convention='.$id_convention.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			//type salarie
				...
				print '<td style="padding-top: 20px; padding-right: 30px"><select name="type_montant" id="type_montant">';
			...
			print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=liste_condition&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'nouvelle_condition_form'){
			...
			$verif_obj = $db->fetch_object($verif_result);	
					print '<div >';
					print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_prime='.$id_prime.'&id_convention='.$id_convention.'">';
					print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&action=liste_condition&id_convention='.$id_convention.'&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
			print '</table>';
				print '</div>';
		...
		if($action == "liste_condition"){
			print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle condition", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=nouvelle_condition_form&id_convention='.$id_convention.'&id_prime='.$id_prime , '', 1), '', 0, 0, 0, 1);
			$id = $id_prime;
			$head = Prime_indm_convention_Head($id, 2, $id_convention);
			print dol_get_fiche_head($head, 'information', "", -1, '');
			...
			//------------------------------------------------------------------------------------------------------------------------------------------
				...
				//------------------------------------------------------------------------------------------------------------------------------------------
						...
						print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?leftmenu=convention&id_convention='.$id_convention.'&action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'&id_convention'.$id_convention.'">'.img_delete('', '').'&nbsp;</a>';								
						print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&id_convention='.$id_convention.'&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'">'.img_edit('', '').'</a></td>';								
						print "<tr>";
						$numero ++;
						$i ++;
				}
