...
//la Troisième page----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   $pdf->SetLineWidth(0.5);
    ...
    $soc_sql = "SELECT nom FROM ".MAIN_DB_PREFIX."societe WHERE rowid IN ".$array_id_soc;
          $res_soc = $db->query($soc_sql);
          if($res_soc){
            $nb = $db->num_rows($res_soc);
            $a = 0;
            while ($a < $nb) {
              if($a == 1 )
                $nom_soc = $db->fetch_object($res_soc)->nom;
              else $nom_soc .= ", ".$db->fetch_object($res_soc)->nom;
    ...
    $action_effectue = "Fiche I.N.P.S par N° affiliation (".$numero.") de : ".$nom_soc." du mois de ".$mois_tab[$mois - 1]." ".$annee." dans société Import|Export";
