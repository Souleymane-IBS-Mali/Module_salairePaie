...
print load_fiche_titre($langs->trans("Ajout d'une grille à l'accord' ".$nom_accord), '', '');
...
if($action == "add"){
	...
	if(empty(GETPOST('nom_categorie', 'int'))){
...
print dol_get_fiche_head($head, 'grille', "", -1, '');		
	...
	<td style="padding-top: 10px" id="nom" ><input id="nom_grille" name ="nom_grille" size="30" type="text" value="'.GETPOST("nom_grille", 'alpha').'" /></td>';
	print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Description</label></td>
	<td style="padding-top: 10px" id="nom" ><input id="commentaire" name ="commentaire" placeholder="Avec la date de création" value="'.GETPOST("commentaire", "alpha").'" size="30" type="text" /></td>';
	print "</tr>";
	...
	print '</form>';
	print '<a class="button" href="./grille_salaire_base.php?mainmenu=paiementsalaire&leftmenu=convention&id_accord='.$id_accord.'&id_convention='.$id_convention.'" >Annuler</a></td></tr>';
	print '</table></div>';
}
