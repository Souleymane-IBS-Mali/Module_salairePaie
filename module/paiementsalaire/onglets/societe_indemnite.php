...
//Titre 
print load_fiche_titre($langs->trans("Listes des indemnités"), '', '');
//print '';
...
if($action == "add_indemnite"){
	if(empty(GETPOST('libelle', 'alpha'))) {
		$message = 'Le champ "LIBELLE" est Obligatoire <br>';
	}
	if(GETPOST('soumis_impot', 'alpha') == "Oui"){
		if(empty(GETPOST("degre_soumission_impot", "float")))
			$message .= 'Veuillez saisir un pourcentage de "SOUMISSION IMPOT" <br>';
	}
	if(GETPOST('soumis_cotisation', 'alpha') == "Oui"){
		if(empty(GETPOST("degre_soumission_cotis", "float")))
	...
	if(empty($message)){
		...
		$degre_s_impot = GETPOST("degre_soumission_impot", "float");
		$degre_s_cotis = GETPOST("degre_soumission_cotis", "float");
		$base_hs = GETPOST("base_hs", "alpha");

		
		$sql_colonne = 'INSERT INTO '.MAIN_DB_PREFIX.'indemnite (libelle, type_indemnite, commentaire, appliquee, exonere, affiche_bulletin, soumis_impot, soumis_cotisation, fk_convention, fk_societe, fk_accord_etablissement, active, ajout_base_hs';
		$sql_valeur = ' VALUES ("'.$libelle.'","'.$type_i.'","'.$desc_i.'",'.$applique.',"'.$exonere.'","'.$affiche_bulletin.'","'.$soumis_impot.'","'.$soumis_cotisation.'",0,'.$id_societe.',0,0,"'.$base_hs.'"';
		if($soumis_impot == "Oui"){
		...
		$result = $db->query($sql);
		if($result){
			$message = "Indemnite Enregistrée avec succès<br><br>Elle doit être configurée dans la liste des indemnite";
			$action = 'detail';
			...
			$action_effectue = "Création d'une indemnité sociéte ".$libelle." de la société ".$obj_soc->nom;
			$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
			$sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Création")';
			$db->query($sql_log);
	...
	if($action == 'supprimer'){
		...
		$sql_select = "SELECT libelle FROM ".MAIN_DB_PREFIX."primes WHERE rowid=".$id_indemnite;
		$obj_i = $db->fetch_object($db->query($sql_select));
		...
		if($result){
			$message = "Indemnite supprimée avec succès";
			//sauvegarde des traces de l'action
			$sql_select = "SELECT firstname, lastname FROM ".MAIN_DB_PREFIX."user WHERE rowid=".$user->id;
			$obj = $db->fetch_object($db->query($sql_select));

			$action_effectue = "Suppression d'une indemnité sociéte ".$obj_i->libelle." de la société ".$obj_soc->nom;
			$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
	...
	$obj_i = $db->fetch_object($db->query($sql_select));
		...
		if($result){
			...
			$action_effectue = "Désactivation d'une indemnité sociéte ".$obj_i->libelle." de la société ".$obj_soc->nom;
			$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
			$sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Désactivation")';
			$db->query($sql_log);
		}else $message = 'Un problème es survenu';
			$action = 'afficher';
	...
	if($action == 'activate'){
		...
		if($num>0){
			...
			if($result){
				...
				$action_effectue = "Activation d'une indemnité sociéte ".$obj_i->libelle." de la société ".$obj_soc->nom;
				$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
				$sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Activation")';
				$db->query($sql_log);
			}else $message = 'Un problème es survenu';
		}else $message = 'Veuillez ajouter au moins un barème pour activer cette indemnite';
	...
	if($action == 'create'){
		print load_fiche_titre($langs->trans("Creation d'une Indemnite"), '', '');
	print '<hr>';

	print '<form name="add_indemnite"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		print '<input type="hidden" name="action" value="add_indemnite">';
		print '<table>';
		print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Libellé</label></td><td style="padding-top: 10px" id="libelle" ><input name ="libelle" value="'.GETPOST("libelle", "alpha").'" size="30" type="text" /></td>';
		...
		print '<tr><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Commentaire</label></td><td style="padding-top: 10px"><textarea name="commentaire" wrap="soft" cols="50" rows="3">'.GETPOST("commentaire", "alpha").'</textarea>
		</td>';
		...
		if(GETPOST("soumis_impot", "alpha"))
			print '<option value="Oui" '.(GETPOST("soumis_impot", "alpha")=="Oui"?"selected":"").'>Oui</option>
				<option value="Non" '.(GETPOST("soumis_impot", "alpha")=="Non"?"selected":"").' >Non</option>';
		else
		...
		if(GETPOST("soumis_cotisation", "alpha"))
			print '<option value="Oui" '.(GETPOST("soumis_cotisation", "alpha")=="Oui"?"selected":"").' >Oui</option>
				<option value="Non" '.(GETPOST("soumis_cotisation", "alpha")=="Non"?"selected":"").' >Non</option>
				</select>';
		else 
			print '<option value="Oui" >Oui</option>
			<option value="Non" selected >Non</option>
		...
		print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Ajouter à la base des Heures sup</label></td><td style="padding-top: 10px">';
		
		print '<select name="base_hs" id="base_hs" >;
		...
		print'</form>';
		print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&action=afficher" class="button">Annuler</a></td></tr>';
		print '</table>';
		print '</div>';
...
if($action == 'afficher'){
	...
	$url = $_SERVER["PHP_SELF"];
		...
		$covSql = "SELECT rowid FROM ".MAIN_DB_PREFIX."accord_etablissement WHERE fk_societe=".$id_societe;
	...
	if($user->rights->paiementsalaire->salarie->write)
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle Indemnité", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&id_convention='.$id_convention.'&action=create&id_societe='.$id_societe , '', 1), '', 0, 0, 0, 1);
	...
	$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."indemnite WHERE fk_convention=".$id_convention." AND active=1";
	if(!empty($id_accord))
	...
	if($result){
		$actl[0] = img_picto($langs->trans("Disabled"), 'switch_off', 'class="size15x"');
		$actl[1] = img_picto("Indemnité heritée", 'switch_on', 'class="size15x"');
		$i = 0;
		$num = $db->num_rows($result);
			while ($i < $num){
				...
				if($obj->active == 1)
					print '<tr class="pair"><td align="left" style="padding: 10px; width : 5%;"><a href="./detail_pr_ind.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&id_convention='.$id_convention.'&id_societe='.$id_societe.'&action=detailindemnite&id_indemnite='.$obj->rowid.'"><b>'.$obj->libelle.'<b></a></td>';
				else
		...
		$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."indemnite WHERE fk_societe=".$id_societe." ORDER BY active DESC";
	$result = $db->query($indSql);//= $db->query($covSql);
	...
	if($result){
		...
		if($user->rights->paiementsalaire->salarie->write){
			while ($i < $num2){
				...
				print '<td>';																			
				print'<a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'&action='.$acts[$obj->active].'&id_indemnite='.$obj->rowid.'&token='.newToken().'">'.$actl[$obj->active].'</a>&nbsp;';
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'&action=detail&id_indemnite='.$obj->rowid.'">'.img_edit().'</a>';	
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'&action=supprimer&id_indemnite='.$obj->rowid.'">'.img_delete().'</a>&nbsp;';
				print "</td></tr>";
				$i ++;
			}
		}else{
		...
		if($num == 0 && $num2 == 0) print "<tr><td align='center' colspan='4'>Auccune Indemnité</td></tr>";
	...
	}else print '<tr><td align="center" colspan="3">Auccune Indemnité Créée!</td></tr>';
}
	...
	if($action == "saveedit_indemnite"){
		...
		if(GETPOST('soumis_cotisation', 'alpha') == "Oui"){
			...
			if(empty($message)){
				...
				$applique = GETPOST('applique');
					$desc_i = empty(GETPOST('commentaire', 'alpha'))? "" : GETPOST('commentaire', 'alpha');
					$sql = 'UPDATE '.MAIN_DB_PREFIX.'indemnite SET libelle="'.$libelle.'", type_indemnite="'.$type_i.'", commentaire="'.$desc_i.'", appliquee="'.$applique.'", exonere="'.$exonere.'", affiche_bulletin="'.$affiche_bulletin.'", soumis_impot="'.$soumis_impot.'", soumis_cotisation="'.$soumis_cotisation.'", ajout_base_hs="'.$base_hs.'"';
					if($soumis_impot == "Oui"){
					...
					if($result){
						...
						$message = "Un problème est survenu !";
						$action = "saveedit_indemnite";
					}
				
			}else $action = "saveedit_indemnite";
		...
		if($action == 'detail'){
			...
			$id = $id_indemnite;
			$head = Prime_indm_societe_Head($id, 1, $id_convention, $id_societe);
			print dol_get_fiche_head($head, 'identifiant', "", -1, '');
		
			$sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."condition_indemnite WHERE fk_indemnite=".$id_indemnite;
			$res = $db->query($sql);
			$obj_cond = $db->fetch_object($res);
			if(!$obj_cond->rowid && $id_indemnite!= 1)
				print "<mark><b>Indemnite ajoutée, veuillez ajouter les conditions d'utilisations dans l'onglet Barèmes.</b></mark>";
			...
			print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><label>Ajouter à la base des heures sup</label></td><td style="width: 150px; padding-top: 10px">'.$obj->ajout_base_hs.'</td></tr>';

			print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'&id_indemnite='.$obj->rowid.'&action=edit_indemnite">Modifier</a></td></tr>';
			print "</table>";
	}
	...
	if($action == 'edit_indemnite'){
		...
		print '<div >';
		print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_indemnite='.$obj->rowid.'&id_societe='.$id_societe.'&id_convention='.$id_convention.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		...
		print '<tr ><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Soumis aux Impôts</label></td><td style="padding-top: 10px; width:80%"">';
		print '<select style="width: 500px; margin-bottom: 10px" name="soumis_impot" id="soumis_impot" >';
		...
		print '<select name="base_hs" id="base_hs" >;';
		if($obj->ajout_base_hs == 'Non')
			print '<option value="Oui" >Oui</option>
				<option value="Non" selected >Non</option>
			</td>';
		else print '<option value="Oui" selected>Oui</option>
			<option value="Non" >Non</option>
			</td>';
		print '</select></tr>';
		...
		print'</form>';
		print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&action=detail&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
		print '</table>';
	...
	if($action == "saveedit_bareme"){
		...
		//-------------------------------
			$sql_insert = "UPDATE ".MAIN_DB_PREFIX."condition_indemnite SET fk_indemnite=".$id_indemnite;
				if($type_montant){
	...
	if($action == 'edit_bareme'){
		...
		print '<div >';
		print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_indemnite='.$id_indemnite.'&rowid_id='.$rowid.'&id_convention='.$id_convention.'&id_societe='.$id_societe.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		...
		print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
		print'</form>';
		print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&action=liste_condition&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
		print '</table>';
	...
	if($action == 'nouvelle_condition_form'){
		...
		$verif_obj = $db->fetch_object($verif_result);	
				print '<div >';
				print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_indemnite='.$id_indemnite.'&id_convention='.$id_convention.'&id_societe='.$id_societe.'">';
				print '<input type="hidden" name="token" value="'.newToken().'">';
				...
				$monform = new Form($db);
				print '<tr><td  style="padding-top: 20px; padding-right: 30px" class="fieldrequired"><label>Type salarie</label></td><td style="width: 300px;">';
				print $monform->multiselectarray('type_salaries', $alltype, GETPOST('indemnite_salarie', 'array'), null, 0, 'quatrevingtpercent widthcentpercentminusx', 0, 0);
				print '</td></tr>';
	...
	if($action == "liste_condition"){
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle condition", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&action=nouvelle_condition_form&id_societe='.$id_societe.'&id_convention='.$id_convention.'&id_indemnite='.$id_indemnite , '', 1), '', 0, 0, 0, 1);
		$id = $id_indemnite;
		$head = Prime_indm_societe_Head($id, 1, $id_convention, $id_societe);
		print dol_get_fiche_head($head, 'information', "", -1, '');
		...
		//------------------------------------------------------------------------------------------------------------------------------------------
			...
			//------------------------------------------------------------------------------------------------------------------------------------------
					...
					print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'&id_societe='.$id_societe.'&id_convention='.$id_convention.'">'.img_delete('', '').'&nbsp;</a>';								
					print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&id_societe='.$id_societe.'&id_convention='.$id_convention.'&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'">'.img_edit('', '').'</a></td>';								
					print "<tr>";
					$numero ++;
					$i ++;
				 if($num == 0) print "<tr><td align='center' colspan='3'>Aucne condition disponible concernant cette Convention</td></tr>";
			}
