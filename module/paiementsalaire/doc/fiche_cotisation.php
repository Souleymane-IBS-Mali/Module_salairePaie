...
class PDF extends FPDF {
    ...
    function Header() {
      ...
      $this->SetTitle("Cotisations",true);
...
function SetCharSpacing($cs) { 
  $this->_out(sprintf('BT %.3F Tc ET',$cs*$this->k)); 
   ...
   $pdf->SetX($x + 69);
   $pdf->SetFont('Arial','',10);
   $pdf->MultiCell(40,3, utf8_decode("N° Employeur : "),0,'C');
...
$pdf->SetFont('Arial','B',6);
   ...
   $y = $pdf->GetY()+23;
    $pdf->SetY($y);
    $pdf->Cell(60,17, "",1,0,'');
    $sql_verif = "SELECT rowid, fk_salarie, salaire_brut FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee." AND mois=".$mois." AND fk_societe=".$id_societe;
    ...
    if($res_verif){
        ...
        while ($i < $num){
                ...
                $sql1 = "SELECT fk_user, matricule FROM ".MAIN_DB_PREFIX."salarie WHERE rowid=".$rowid_bulletin[$i]->fk_salarie;
								$res = $db->query($sql1);
								if($res){
									$liste_salarie[$i] = $db->fetch_object($res);
									$user_Sql = "SELECT rowid, firstname, lastname FROM ".MAIN_DB_PREFIX."user where rowid=".$liste_salarie[$i]->fk_user;
									$user_Result = $db->query($user_Sql);
                  if($user_Result){
                    $liste_user[$i] = $db->fetch_object($user_result);
   ...
   $pdf->SetX(30);
   $pdf->MultiCell(5,3, utf8_decode($nb),0,'L');
...
$pdf->Cell(10,4, utf8_decode("Autres"),0,0,'L');
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->Cell(15,12, utf8_decode("N° Assurés"),0,0,'C');

   $pdf->SetX(20);
   $pdf->Cell(35,12, utf8_decode("Nom"),0,0,'C');
   
   $pdf->SetX(55);
   $pdf->MultiCell(15,12, utf8_decode("Numéro AMO"),0,'C');
   ...
   $pdf->SetX(74);
   $pdf->MultiCell(4,4, utf8_decode("Mois"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(78);
   $pdf->MultiCell(9,4, utf8_decode("Date entrée"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(87);
   $pdf->MultiCell(9,4, utf8_decode("Date sortie"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(96);
   $pdf->MultiCell(12,4, utf8_decode("Brut"),0,'C');
    /*  $x = 108;
   $sql_type_cotis = "SELECT rowid, code FROM ".MAIN_DB_PREFIX."type_prestation";
   $res_type_cotis = $db->query($sql_type_cotis);
   if($res_type_cotis){
    $nb = $db->num_rows($res_type_cotis);
    $a = 0;
    while ($a < $nb) {
      $obj_code_type_cotis = $db->fetch_object($res_type_cotis);
      $pdf->SetY($y);
      $pdf->SetX($x);
      $pdf->MultiCell(12,3, utf8_decode($obj_code_type_cotis->code),0,'C');
   ...
   $pdf->SetX(108);
    if($id_convention == 1)
      $pdf->MultiCell(12,3, utf8_decode("Accident du Travail 4%"),0,'C');
    else  $pdf->MultiCell(12,3, utf8_decode("Accident du Travail 2%"),0,'C');


   $pdf->SetY($y);
   $pdf->SetX(120);
   $pdf->MultiCell(12,3, utf8_decode("Prestations Familiales 8%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(132);
   $pdf->MultiCell(12,3, utf8_decode("Rétraite 7%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(144);
   $pdf->MultiCell(13,3, utf8_decode("Ivalidité Allocations Survivant 2%"),1,'C');

   $pdf->SetY($y);
   $pdf->SetX(157);
   $pdf->MultiCell(12,3, utf8_decode("ANPE 1%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(169);
   $pdf->MultiCell(12,3, utf8_decode("AMO employeur 3.5%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(181);
   $pdf->MultiCell(12,3, utf8_decode("AMO employé 3.06%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(193);
   $pdf->MultiCell(13,3, utf8_decode("Total Cotisations"),0,'C');
...
if($nb>0){
   for ($i=0; $i < $nb; $i++) {
     if($i>0 && $i%35 == 0){
      $y = $pdf->GetY()+1;
      $pdf->line(20,$y_rect,20,$y_rect+($y - $y_rect+3));
      $pdf->line(55,$y_rect,55,$y_rect+($y - $y_rect+3));
      $pdf->line(70,$y_rect,70,$y_rect+($y - $y_rect+3));
      $pdf->line(74,$y_rect,74,$y_rect+($y - $y_rect+3));
      $pdf->line(78,$y_rect,78,$y_rect+($y - $y_rect+3));
      $pdf->line(87,$y_rect,87,$y_rect+($y - $y_rect+3));
      $pdf->line(96,$y_rect,96,$y_rect+($y - $y_rect+3));
      $pdf->line(108,$y_rect,108,$y_rect+($y - $y_rect+3));
      $pdf->line(120,$y_rect,120,$y_rect+($y - $y_rect+3));
      $pdf->line(132,$y_rect,132,$y_rect+($y - $y_rect+3));
      $pdf->line(144,$y_rect,144,$y_rect+($y - $y_rect+3));
      $pdf->line(157,$y_rect,157,$y_rect+($y - $y_rect+3));
      $pdf->line(169,$y_rect,169,$y_rect+($y - $y_rect+3));
      $pdf->line(181,$y_rect,181,$y_rect+($y - $y_rect+3));
      $pdf->line(193,$y_rect,193,$y_rect+($y - $y_rect+3));

      $pdf->SetY($y_rect);
      $pdf->SetX(5);
     $pdf->Cell($pdf->GetPageWidth()-9.5,$y - $y_rect + 3, "",1,0,'');
      ...
      $y = $pdf->GetY() +6; 
              ...
              $pdf->SetFont('Arial','B',6);
              $pdf->SetX(5);
              $pdf->Cell(15,4, utf8_decode($liste_salarie[$i]->matricule?:""),0,0,'L');

              $pdf->SetX(20);
              $pdf->Cell(35,4, utf8_decode($liste_user[$i]->firstname." ".$liste_user[$i]->lastname),0,0,'L');
              ...
              $sql_cotis = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin_cotisation WHERE fk_bulletin=".$rowid_bulletin[$i]->rowid;//AND fk_cotisation=".$id_cotisation;
            ...
            if($id_convention == 1)
              $taux_accident = 4;
            $pdf->SetX(96);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut,  0)),0,0,'R');
              $total_brut += $rowid_bulletin[$i]->salaire_brut;
              $pdf->SetX(108);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut*$taux_accident/100,  0)),0,0,'R');
              $total_horizontal += $rowid_bulletin[$i]->salaire_brut*$taux_accident/100;
              $total_acc += $rowid_bulletin[$i]->salaire_brut*$taux_accident/100;

              $pdf->SetX(120);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut*8/100,  0)),0,0,'R');
              $total_horizontal += $rowid_bulletin[$i]->salaire_brut*8/100;
              $total_prst += $rowid_bulletin[$i]->salaire_brut*8/100;

              $pdf->SetX(132);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut*7/100,  0)),0,0,'R');
              $total_horizontal += $rowid_bulletin[$i]->salaire_brut*7/100;
              $total_retraite += $rowid_bulletin[$i]->salaire_brut*7/100;

              $pdf->SetX(144);
              $pdf->Cell(13,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut*2/100,  0)),0,0,'R');
              $total_horizontal += $rowid_bulletin[$i]->salaire_brut*2/100;
              $total_alloc += $rowid_bulletin[$i]->salaire_brut*2/100;;

              $pdf->SetX(157);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($rowid_bulletin[$i]->salaire_brut/100,  0)),0,0,'R');
              $total_horizontal += $rowid_bulletin[$i]->salaire_brut/100;
              $total_anpe += $rowid_bulletin[$i]->salaire_brut/100;

              $pdf->SetX(169);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($cotis->montant_employe, 0)),0,0,'R');
              $total_horizontal += $cotis->montant_employe;
              $total_amo_sal += $cotis->montant_employe;

              $pdf->SetX(181);
              $pdf->Cell(12,4, utf8_decode(apres_virgule($cotis->montant_employeur, 0)),0,0,'R');
              $total_horizontal += $cotis->montant_employeur;
              $total_amo_patro += $cotis->montant_employeur;

              $pdf->SetY($y+0.5);
              $pdf->SetX(193);
              $pdf->Cell(12.5,3.5, utf8_decode(apres_virgule($total_horizontal, 0)),0,0,'R',true);
              $total_total_horizotal += $total_horizontal;
            
     		//if($i%34 != 0)
             $pdf->line(5,$y+4,$pdf->GetPageWidth()-5,$y+4);
   ...
   if(($pdf->GetPageWidth()-$y)<50){
    ...
    $pdf->SetFont('Arial','',5.5);
            $pdf->SetX(96);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_brut,  0)),0,0,'R',true);

              $pdf->SetX(108);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_acc,  0)),0,0,'R',true);

              $pdf->SetX(120);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_prst,  0)),0,0,'R',true);

              $pdf->SetX(132);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_retraite,  0)),0,0,'R',true);

              $pdf->SetX(144);
              $pdf->Cell(12.5,5, utf8_decode(apres_virgule($total_alloc,  0)),0,0,'R',true);

              $pdf->SetX(157);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_anpe,  0)),0,0,'R',true);

              $pdf->SetX(169);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_amo_sal, 0)),0,0,'R',true);

              $pdf->SetX(181);
              $pdf->Cell(11.5,5, utf8_decode(apres_virgule($total_amo_patro, 0)),0,0,'R',true);
        
              $pdf->SetX(193);
              $pdf->Cell(12.5,5, utf8_decode(apres_virgule($total_total_horizotal, 0)),0,0,'R',true);

                $y = $pdf->GetY()+5;
                $pdf->line(20,$y_rect,20,$y_rect+($y - $y_rect-6));
                $pdf->line(55,$y_rect,55,$y_rect+($y - $y_rect-6));
                $pdf->line(70,$y_rect,70,$y_rect+($y - $y_rect-6));
                $pdf->line(74,$y_rect,74,$y_rect+($y - $y_rect-6));
                $pdf->line(78,$y_rect,78,$y_rect+($y - $y_rect-6));
                $pdf->line(87,$y_rect,87,$y_rect+($y - $y_rect-6));
                $pdf->line(96,$y_rect,96,$y_rect+($y - $y_rect));
                $pdf->line(108,$y_rect,108,$y_rect+($y - $y_rect));
                $pdf->line(120,$y_rect,120,$y_rect+($y - $y_rect));
                $pdf->line(132,$y_rect,132,$y_rect+($y - $y_rect));
                $pdf->line(144,$y_rect,144,$y_rect+($y - $y_rect));
                $pdf->line(157,$y_rect,157,$y_rect+($y - $y_rect));
                $pdf->line(169,$y_rect,169,$y_rect+($y - $y_rect));
                $pdf->line(181,$y_rect,181,$y_rect+($y - $y_rect));
                $pdf->line(193,$y_rect,193,$y_rect+($y - $y_rect));
  ...
  $pdf->SetY($y);
    ...
    $pdf->SetFont('Arial','',5.5);
            $pdf->SetX(96);
              $pdf->Cell(11.5,5, utf8_decode(0),0,0,'R',true);
              ...
              $pdf->SetX(144);
              $pdf->Cell(12.5,5, utf8_decode(0),0,0,'R',true);
...
$pdf->SetFont('Arial','',8);
   ...
   $pdf->SetX(65);
   $pdf->Cell(20,3, utf8_decode(apres_virgule($total_total_horizotal, 0)),0,0,'R'); 
   ...
   $pdf->SetX(65);
   $pdf->Cell(20,3, utf8_decode("0"),0,0,'R');
...
$pdf->SetFont('Arial','B',6);
   ...
   $pdf->SetY($y+1);
   $pdf->SetX(98);
   $pdf->Cell(20,3, utf8_decode("Perm. + Expat"),0,0,'L');

   $pdf->SetX(118);
   $pdf->Cell(20,3, utf8_decode("Occasionnels"),0,0,'L');

   $pdf->line(118,$y,118,$y+15);

   $pdf->line(88,$y+5,88,$y+15);
   
   $pdf->line(88,$y+5,138,$y+5);

   $pdf->line(88,$y+10,138,$y+10);

   $pdf->line(88,$y+15,98,$y+15);

   //-------------
   $pdf->SetY($y+6);
    $pdf->SetX(88);
    $pdf->Cell(10,3, utf8_decode("Eff."),0,0,'L');

    $pdf->SetX(98);
    $pdf->Cell(20,3, utf8_decode($nb),0,0,'C');

    $pdf->SetX(116);
    $pdf->Cell(20,3, utf8_decode(0),0,0,'C');
    ...
    $pdf->SetX(98);
    $pdf->Cell(20,3, utf8_decode(apres_virgule($total_brut, 0)),0,0,'R');
    ...
    $pdf->SetY($y);
    $pdf->SetX(140);
    $pdf->Cell(30,4, utf8_decode("INPS :    ".apres_virgule($total_total_horizotal-$total_amo_sal-$total_amo_sal, 0)),0,0,'L', true);

    //AMO
    $pdf->SetY($y);
    $pdf->SetX(172);
    $pdf->MultiCell(30,4, utf8_decode("AMO :   ".apres_virgule($total_amo_sal+$total_amo_sal, 0)),0,'L', true);

    //Exactitude de la déclaration
    $pdf->SetFont('Arial','',8);
    $pdf->SetY($y+9);
    $pdf->SetX(140);
    $pdf->MultiCell(70,4, utf8_decode("Je soussigne, certifie exacte la présente déclaration "),0,'L', false);
    ...
    $pdf->SetFont('Arial','',6);
    $pdf->SetX(15);
    $pdf->MultiCell(100,3, utf8_decode("- Exceptés les frais professionnels, toute rémunération (espèces, avantage en nature) portée dans la comptabilité sous quelque rubrique que ce soit doit être soumise à cotisations."),0,'L');
    
    $pdf->SetY($y+6);
    $pdf->SetLeftMargin(5);
    $pdf->SetX(15);
    $pdf->MultiCell(100,3, utf8_decode("- Le non paiement des cotisations à l'échéance ne dispense pas de l'employeur de la déclarations des cotisations dues. Art. 191 du CPS, art 107 du décret 09-552 P-RM du 12 Octobre 2009."),0,'L');
...
//la deuxième page----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   $pdf->SetX($x+5);
   $pdf->SetFont('Arial','',10);
   $pdf->MultiCell(52,3, utf8_decode("N° Employeur : "),0,'L');
...
//3ème Rectangle-------------------------------------------------------------------------------
  ...
  //$y_cocher_valider = $x+3.5;
   ...
   $pdf->Cell(10,4, utf8_decode("Autres"),0,0,'L');
    ...
    $pdf->SetLeftMargin(5);
    $pdf->MultiCell(90,3, utf8_decode($titre),0,'L',0);

    $pdf->SetFont('Arial','',7);
    $y = $pdf->GetY();
    $pdf->SetY($y+2);
    $pdf->SetX(17);
    $pdf->Cell(70,3, utf8_decode("- Taux des cotisations :       20% (compris entre 19% et 22%"),0,0,'L',0);
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->MultiCell(35,4, utf8_decode("Libellé des cotisations sociales"),0,'C');
   
   $pdf->SetY($y);
   $pdf->SetX(40);
   $pdf->MultiCell(15,4, utf8_decode("Effectif"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(55);
   $pdf->MultiCell(20,4, utf8_decode("Masse salariale"),0,'C');


   $pdf->SetY($y);
   $pdf->SetX(75);
   $pdf->MultiCell(20,4, utf8_decode("Rétraite 7%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(95);
   $pdf->MultiCell(20,3, utf8_decode("Ivalidité Allocations Survivant 2%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(115);
   $pdf->MultiCell(20,3, utf8_decode("Prestations Familiales 8%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(135);
   $pdf->MultiCell(20,3, utf8_decode("Accident du travail 2%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(155);
   $pdf->MultiCell(20,3, utf8_decode("ANPE 1%"),0,'C');

   $pdf->SetY($y);
   $pdf->SetX(175);
   $pdf->MultiCell(30,3, utf8_decode("Montant des Cotisations"),0,'C');
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->MultiCell(35,4, utf8_decode("Taux de cotisations"),0,'C');
   
   $pdf->SetY($y);
   $pdf->SetX(40);
   $pdf->MultiCell(15,4, utf8_decode(""),0,'R');
   ...
   $pdf->SetY($y);
   $pdf->SetX(75);
   $pdf->MultiCell(20,4, utf8_decode("7%"),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(95);
   $pdf->MultiCell(20,3, utf8_decode("2%"),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(115);
   $pdf->MultiCell(20,3, utf8_decode("8%"),0,'R');
   ...
   $pdf->SetY($y);
   $pdf->SetX(175);
   $pdf->MultiCell(30,3, utf8_decode("Total Cotisations"),0,'C');
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->MultiCell(35,4, utf8_decode("Salariés permanents"),0,'C');
   
   $pdf->SetY($y);
   $pdf->SetX(40);
   $pdf->MultiCell(15,4, utf8_decode($nb),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(55);
   $pdf->MultiCell(20,4, utf8_decode(apres_virgule($total_brut, 0)),0,'R');


   $pdf->SetY($y);
   $pdf->SetX(75);
   $pdf->MultiCell(20,4, utf8_decode(apres_virgule($total_retraite, 0)),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(95);
   $pdf->MultiCell(20,3, utf8_decode(apres_virgule($total_alloc, 0)),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(115);
   $pdf->MultiCell(20,3, utf8_decode(apres_virgule($total_prst, 0)),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(135);
   $pdf->MultiCell(20,3, utf8_decode(apres_virgule($total_acc, 0)),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(155);
   $pdf->MultiCell(20,3, utf8_decode(apres_virgule($total_anpe, 0)),0,'R');

   $pdf->SetY($y);
   $pdf->SetX(175);
   $pdf->MultiCell(30,3, utf8_decode(apres_virgule($total_total_horizotal-$total_amo_sal-$total_amo_sal, 0)),0,'R');
   $pdf->line(5,$pdf->GetY()+2,$pdf->GetPageWidth()-5,$pdf->GetY()+2);
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->MultiCell(35,4, utf8_decode("Salariés occasionnels"),0,'C');
   
   $pdf->SetY($y);
   $pdf->SetX(40);
   $pdf->MultiCell(15,4, utf8_decode(0),0,'R');
   ...
   $pdf->SetY($y);
   $pdf->SetX(95);
   $pdf->MultiCell(20,3, utf8_decode(0),0,'R');
   ...
   $pdf->SetY($y);
   $pdf->SetX(175);
   $pdf->MultiCell(30,3, utf8_decode(0),0,'R');
   $pdf->line(5,$pdf->GetY()+2,$pdf->GetPageWidth()-5,$pdf->GetY()+2);
   ...
   $pdf->SetY($y);
   $pdf->SetX(5);
   $pdf->MultiCell(35,4, utf8_decode("Total des cotisations(1)"),0,'C');
   ...
   $y = $pdf->GetY()+2;
   $pdf->line(40,$y_rect,40,$y_rect+($y - $y_rect));
    $pdf->line(55,$y_rect,55,$y_rect+($y - $y_rect));
    $pdf->line(75,$y_rect,75,$y_rect+($y - $y_rect));
    $pdf->line(95,$y_rect,95,$y_rect+($y - $y_rect));
    $pdf->line(115,$y_rect,115,$y_rect+($y - $y_rect));
    $pdf->line(135,$y_rect,135,$y_rect+($y - $y_rect));
    $pdf->line(155,$y_rect,155,$y_rect+($y - $y_rect));
    $pdf->line(175,$y_rect,175,$y_rect+($y - $y_rect));
