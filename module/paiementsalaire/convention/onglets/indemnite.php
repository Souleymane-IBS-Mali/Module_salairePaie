...
$titre = "Indemnités de la convention ".$nom_convention;
print load_fiche_titre($langs->trans($titre), '', '');
//print '<hr>';

$head = paiementsalaireConventionHead($id_convention);
print dol_get_fiche_head($head, 'indemnite', "", -1, '');
...
if(!empty(GETPOST("rowid_id", "int")))
	...
	if($action == "add_indemnite"){
		...
		if(empty($message)){
			...
			$sql_colonne = 'INSERT INTO '.MAIN_DB_PREFIX.'indemnite (libelle, type_indemnite, commentaire, appliquee, exonere, affiche_bulletin, soumis_impot, soumis_cotisation, fk_convention, fk_accord_etablissement, fk_societe, active';
			$sql_valeur = ' VALUES ("'.$libelle.'","'.$type_i.'","'.$desc_i.'",'.$applique.',"'.$exonere.'","'.$affiche_bulletin.'","'.$soumis_impot.'","'.$soumis_cotisation.'",'.$id_convention.',0,0,0';
			if($soumis_impot == "Oui"){
		...
		print '<form name="add_indemnite"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'">';
			...
			print '<div id="container_degre_soumission_impot">
				<input type="number" name="degre_soumission_impot" min="1" max="100" value="'.(GETPOST("degre_soumission_impot", "float")?:"100").'" >%
			</div>';
			print '</td>';
			print '</tr>';	
	...
	if(GETPOST("soumis_cotisation", "alpha"))
		...
		<option value="Non" selected >Non</option>
			...
			print '<div id="container_degre_soumission_cotis">
				<input type="number" name="degre_soumission_cotis" min="1" max="100" value="'.(GETPOST("degre_soumission_cotis", "float")?:"100").'" >%
			</div>';
			print '</td>';
			print '</tr>';	
			...
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=afficher" class="button">Annuler</a></td></tr>';
			print '</table>';
			print '</div>';
		...
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle Indemnité", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=create&id_convention='.$id_convention , '', 1), '', 0, 0, 0, 1);
		...
		$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."indemnite WHERE fk_convention=".$id_convention." ORDER BY active DESC";
		...
		if($result){
			...
			while ($i < $num){
				...
				if($obj->active == 1)
					print '<tr class="pair"><td align="left" style="padding: 10px; width : 5%;"><a href="./detail_pr_ind.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=detailindemnite&id_indemnite='.$obj->rowid.'"><b>'.$obj->libelle.'<b></a></td>';
				else
				...
				print '<td>';																			
				print'<a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action='.$acts[$obj->active].'&id_indemnite='.$obj->rowid.'&token='.newToken().'">'.$actl[$obj->active].'</a>&nbsp;';
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=detail&id_indemnite='.$obj->rowid.'">'.img_edit().'</a>';	
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=supprimer&id_indemnite='.$obj->rowid.'">'.img_delete().'</a>&nbsp;';
				print "</td></tr>";
				$i ++;
			}
			$db->free($result);

			if($num == 0) print "<tr><td align='center' colspan='4'>Auccune Indemnité</td></tr>";
			print "<script>
			function myFunction(e){
			   var b = 'delete'+e;
			   var button_generer = document.getElementById(b);
			   if(!confirm('Click sur OK pour confirmer cette suppression')){
				   var lien = '".$_SERVER["PHP_SELF"]."?mainmenu=paiementsalaire&leftmenu=convention&id_convention=".$id_convention."&action=afficher';
		...
		if($action == "saveedit_indemnite"){
			...
			if($action == 'detail'){
				...
				$id = $id_indemnite;
				$head = Prime_indm_convention_Head($id, 1, $id_convention);
				...
				print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&id_indemnite='.$obj->rowid.'&action=edit_indemnite">Modifier</a></td></tr>';
				print "</table>";
		}
		...
		if($action == 'edit_indemnite'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_indemnite='.$obj->rowid.'&id_convention='.$id_convention.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=detail&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'edit_bareme'){
			...
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_indemnite='.$id_indemnite.'&rowid_id='.$rowid.'&id_convention='.$id_convention.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			//type salarie
				...
				if($condit->type_montant == "forfait"){
					...
					print '<option value="pourcentage">Pourcentage</option><option value="flottante" >Flottante</option></select></td></td></tr>';
				}else if($condit->type_montant == "pourcentage"){
					print '<option value="forfait">Forfait</option>';
					print '<option value="pourcentage" selected>Pourcentage</option><option value="flottante" >Flottante</option></select></td></td></tr>';
				}else{
					print '<option value="forfait">Forfait</option>';
					print '<option value="pourcentage" selected>Pourcentage</option><option value="flottante" selected>Flottante</option></select></td></td></tr>';
				}
			...
			print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_convention='.$id_convention.'&action=liste_condition&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		if($action == 'nouvelle_condition_form'){
			...
			$verif_obj = $db->fetch_object($verif_result);	
					print '<div >';
					print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&id_indemnite='.$id_indemnite.'&id_convention='.$id_convention.'">';
					print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=convention&action=liste_condition&id_convention='.$id_convention.'&id_indemnite='.$id_indemnite.'" class="button">Annuler</a></td></tr>';
			print '</table>';
				print '</div>';
		...
		if($action == "liste_condition"){
			print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle condition", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=convention&action=nouvelle_condition_form&id_convention='.$id_convention.'&id_indemnite='.$id_indemnite , '', 1), '', 0, 0, 0, 1);
			$id = $id_indemnite;
			$head = Prime_indm_convention_Head($id, 1, $id_convention);
			print dol_get_fiche_head($head, 'information', "", -1, '');
			$verif_sql = "SELECT * FROM ".MAIN_DB_PREFIX."indemnite WHERE rowid=".$id_indemnite;
			...
			//------------------------------------------------------------------------------------------------------------------------------------------
				...
				//------------------------------------------------------------------------------------------------------------------------------------------
						...
						print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?leftmenu=convention&id_convention='.$id_convention.'&action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'&id_convention'.$id_convention.'">'.img_delete('', '').'&nbsp;</a>';								
						print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&id_convention='.$id_convention.'&rowid_id='.$obj_ind->rowid.'&id_indemnite='.$obj->rowid.'">'.img_edit('', '').'</a></td>';								
						print "<tr>";
						$numero ++;
						$i ++;
				}
