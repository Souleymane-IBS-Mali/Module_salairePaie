...
$obj_salarie = $db->fetch_object($res);
  ...
  global $fk_salarie, $fk_user, $y, $mois, $annee, $id_accord_etab, $societe_Salarie, $info_soc;
}
...
if($action == "no_save"){
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      ...
      return pdf_pagehead($this, "onglet_salarie");
    }
    ...
    $y = $pdf->GetY() +6;
    $pdf->SetY($y);
    $pdf->Cell(30,4, utf8_decode("Taux Horaires"),0,0,'L');
    $pdf->Cell(20,4, utf8_decode($heur_normal),0,0,'R');
*/
    ...
    $pdf->SetLeftMargin(103);
		...
		if(!empty($key) && !empty($value)){
			...
			if(!empty($key) && !empty($value)){
				...
				if($ind_res){
                  ...
                  $pdf->SetLeftMargin(63);
                  $pdf->Cell(20,4, utf8_decode(round($pourcentage_ind[$index]*$base_pourcentage)."%"),0,0,'R');
		...
		foreach ($ind as $key => $value) {
		  if(!empty($key) && !empty($value)){
			...
			if($ind_res){
              ...
              $pdf->SetLeftMargin(63);
              if(round($base_pourcentage*100,2) == 100)
                $p = 100;
              else $p = round($base_pourcentage*100, 2);
      ...
      $pdf->SetLeftMargin(161);
      $pdf->MultiCell(31,4, utf8_decode(apres_virgule($db, $id_societe, round($salaire_net), 2)),0,'R','true');
  ...
  class PDF extends FPDF {
    ...
    $pdf->Cell(35,4, utf8_decode("Salaire de base normale"),0,0,'L');
      ...
      if($bulletin_pr_res){
        ...
        while ($i < $num){
          ...
          if ($obj_bulletin_pr->affiche_bulletin)
          {
            ...
            $pdf->SetLeftMargin(83);
            $pdf->Cell(20,4, utf8_decode(apres_virgule($db, $id_societe, ($obj_bulletin_pr->montant*100/$obj_bulletin_pr->pourcentage), 2)),0,0,'R'); //On le divise pour avoir le montant exacte

            $pdf->SetLeftMargin(103);
            $pdf->Cell(30,4, utf8_decode(apres_virgule($db, $id_societe, $obj_bulletin_pr->montant, 2)),0,0,'R');//on a le montant en pourcentage
      ...
      else $pdf->Cell(20,4, utf8_decode(apres_virgule($db, $id_societe, ($obj_bulletin->sursalaire)?:0, 2)),0,0,'R');
      ...
      if($bulletin_hs_res){
        ...
        while ($m < $num){
          ...
          $pdf->SetX(83);
          $pdf->Cell(20,4, utf8_decode(apres_virgule($db, $md_societe, $obj_bulletin_hs->base, 2)),0,0,'R');
      ...
      $pdf->SetY($y);
      $pdf->SetFillColor(245, 245, 245);
      $pdf->MultiCell(65,4, utf8_decode("Numéro de compte : ".$obj_bulletin->compte),0,'L');
    ...
    //Avance
      ...
      if(is_readable($filePath) && $obj_bulletin->cloture == "oui"){
        $pdf->Image($filePath,150,$y+7, 20,13);
      }
    }
