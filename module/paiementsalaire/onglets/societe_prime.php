...
//Titre
print load_fiche_titre($langs->trans("Listes des primes"), '', '');
//print '';
...
if($action == "add_prime"){
	...
	if(empty($message)){
		...
		$montant = 0;
		$montant_excep = GETPOST('montant', 'int');
		$type_i = GETPOST('type');
		...
		if($type_i == "exceptionnelle"){//La primes est exceptionnelle
			...
			$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'primes (libelle, type_prime, commentaire, appliquee, exonere, affiche_bulletin, soumis_impot, soumis_cotisation, fk_convention, fk_societe, fk_accord_etablissement, active, ajout_base_hs) VALUES ("'.$libelle.'","'.$type_i.'","'.$desc_i.'",'.$applique.',"'.$exonere.'","'.$affiche_bulletin.'","'.$soumis_impot.'","'.$soumis_cotisation.'",0,'.$id_societe.',0,0,"'.$base_hs.'")';
			$result = $db->query($sql);
			if($result){
				...
				$action_effectue = "Création d'une prime sociéte ".$libelle." de la société ".$obj_soc->nom;
				$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
				$sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Ajout")';
				$db->query($sql_log);
			}else{
	...
	if($action == 'supprimer'){
		...
		$sql_select = "SELECT libelle FROM ".MAIN_DB_PREFIX."primes WHERE rowid=".$id_prime;
		$obj_p = $db->fetch_object($db->query($sql_select));
		...
		if($result){
			...
			$action_effectue = "Suppression d'une prime sociéte ".$obj_p->libelle." de la société ".$obj_soc->nom;
			$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
	...
	if($action == 'disable'){
		...
		if($result){
			...
			$action_effectue = "Désactivation d'une prime sociéte ".$obj_p->libelle." de la société ".$obj_soc->nom;
			$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
	...
	if($action == 'activate'){
		...
		if($num>0){
			...
			if($result){
				...
				$action_effectue = "Activation d'une prime sociéte ".$obj_p->libelle." de la société ".$obj_soc->nom;
				$sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
	...
	if($action == 'create'){
		...
		print '<form name="add_prime"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'">';
		...
		print '<input type="radio" value="obligatoire" id="obligatoire" name="type" id="oblig" ><label for="oblig" >Obligatoire&ensp;</label>
		<input type="radio" value="facultative" id="facultatif" name="type" id="fac" checked><label for="fac" >Facultative&ensp;</label>
		<input type="radio" value="exceptionnelle" name="type" id="exceptionnelle"><label for="fac" >Exceptionnelle&ensp;</label>'.img_picto('Attention : Si vous cochez exceptionnelle, cette prime sera exceptionnelle et elle sera affecter à tous les salariés de -->'.$obj_soc->nom.'<--','error');

		//Section à faire disparaître si exceptionnelle est cochée
		print '<div id = "non_exceptionnelle">';
		//Commentaire
			print '<tr><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Commentaire</label></td><td style="padding-top: 10px"><textarea id = "commentaire" name="commentaire" wrap="soft" cols="50" rows="3">'.GETPOST("commentaire").'</textarea></td></tr>';
		//Montant
			print '<tr><td style="padding-top: 10px; padding-right: 30px" class="fieldrequired"><label>Montant</label></td><td style="padding-top: 10px"><input type="text" value="'.GETPOST("montant").'" name="montant" id = "montant" ></td></tr>';
		//S'applique au
		...
		//Rétirer du Salaire de base
			...
			print '<input id = "retire_sal_base1" type="radio" value="oui" name="exonere" checked><label for="oui" >Oui&ensp;</label>
				<input type="radio" id = "retire_sal_base2" value="non" name="exonere" ><label for="non" >Non&ensp;</label></td></tr>';
		//Afficher sur bulletin
		...
		//Ajouter à la base des heures sup
		...
		print '<script type="text/javascript">
			var obligatoire = document.getElementById("obligatoire");
			var facultatif = document.getElementById("facultatif");
			var excep = document.getElementById("exceptionnelle");

			var montant = document.getElementById("montant");
			var base_hs = document.getElementById("base_hs");
			var applique = document.getElementById("applique");
			var retire_sal_base1 = document.getElementById("retire_sal_base1");
			var retire_sal_base2 = document.getElementById("retire_sal_base2");

			//Cache le champ montant de la prime exceptionnelle
			montant.disabled = true;
			obligatoire.addEventListener("change", function () {
				montant.disabled = true;
				base_hs.disabled = false;
				applique.disabled = false;
				retire_sal_base1.disabled = false;
				retire_sal_base2.disabled = false;
			},
			false,
			);

			//Cache le champ montant de la prime exceptionnelle
			facultatif.addEventListener("change", function () {
				montant.disabled = true;
				base_hs.disabled = false;
				applique.disabled = false;
				retire_sal_base1.disabled = false;
			...
			//Cachant les parties non concerné par la prime exceptionnele
		  	excep.addEventListener("change", function () {
				montant.disabled = false;
				base_hs.disabled = true;
				applique.disabled = true;
				retire_sal_base1.disabled = true;
				retire_sal_base2.disabled = true;
			},
			false,
			);
...
if($action == 'afficher'){
	...
	if($user->rights->paiementsalaire->salarie->write)
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle Prime", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&action=create&id_convention='.$id_convention.'&id_societe='.$id_societe , '', 1), '', 0, 0, 0, 1);
	...
	if($result){
		...
		$actl[1] = img_picto("Prime heritée", 'switch_on', 'class="size15x"');
		$i = 0;
		$num = $db->num_rows($result);
		while ($i < $num){
			$obj = $db->fetch_object($result);
			...
			if($obj->active == 1)
				print '<tr class="pair"><td align="left" style="padding: 10px; width : 5%;"><a href="./detail_pr_ind.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&id_convention='.$id_convention.'&id_societe='.$id_societe.'&action=detailprime&id_prime='.$obj->rowid.'"><b>'.$obj->libelle.'<b></a></td>';
			else
	...
	if($result){
		...
		if($user->rights->paiementsalaire->salarie->write){
			...
			if($result_s){
				$nb_pr_except = $db->num_rows($result_s);
				$a = 0;
				while ($a < $nb_pr_except) {
					...
					$compter = 0;
					if($result_s2){
						$nb_pr_except2 = $db->num_rows($result_s2);
						$b = 0;
						while ($b < $nb_pr_except2) {
							$obj_sal2 = $db->fetch_object($result_s2);
							if($obj_sal->libelle == $obj_sal2->libelle && $obj_sal->montant == $obj_sal2->montant
								&& $obj_sal->commentaire == $obj_sal2->commentaire && $obj_sal->date_limit == $obj_sal2->date_limit){
								//On compte le nombre de salarié qui bénéficie cette prime
								$compter ++;
							}
							$b ++;
						}
					...
					if($compter == $nb_sal){ //si le nombre de salarié compté égal au nombre total de salarié on affcihe la prime
						...
						for ($i=0; $i < count($tab_aff); $i++) {
							if($obj_sal->libelle == $tab_aff[$i]->libelle && $obj_sal->montant == $tab_aff[$i]->montant
								&& $obj_sal->commentaire == $tab_aff[$i]->commentaire && $obj_sal->date_limit == $tab_aff[$i]->date_limit){

								$trouve = false;
							}
						}

						if($trouve || count($tab_aff) == 0){
							$date = explode('-', $obj_sal->date_limit); //annee-mois-jours

							$sql_bulletin = "SELECT rowid, annee, mois FROM ".MAIN_DB_PREFIX."bulletin WHERE cloture='non' AND fk_societe=".$id_societe. " AND annee=".$date[0]." AND mois=".$date[1]." ORDER BY rowid DESC";
							$res_bulletin = $db->query($sql_bulletin);
	...
	if($action == "saveedit_prime"){
		...
		if(empty(GETPOST('libelle', 'alpha'))) {
			...
			if(empty($message)){
				...
				$applique = GETPOST('applique');
					...
					$sql = 'UPDATE '.MAIN_DB_PREFIX.'primes SET libelle="'.$libelle.'", type_prime="'.$type_i.'", commentaire="'.$desc_i.'", appliquee="'.$applique.'", exonere="'.$exonere.'", affiche_bulletin="'.$affiche_bulletin.'", soumis_impot="'.$soumis_impot.'", soumis_cotisation="'.$soumis_cotisation.'", ajout_base_hs="'.$base_hs.'" WHERE rowid='.$id_prime;
					$result = $db->query($sql);
		...
		if($action == 'detail'){
			...
			$id = $id_prime;
			$head = Prime_indm_societe_Head($id, 2, $id_convention, $id_societe);
			...
			print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_societe='.$id_societe.'&id_convention='.$id_convention.'&id_prime='.$obj->rowid.'&action=edit_prime">Modifier</a></td></tr>';
			print "</table>";
	}
	...
	if($action == 'edit_prime'){
		...
		print '<div >';
		print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_prime='.$obj->rowid.'&id_societe='.$id_societe.'&id_convention='.$id_convention.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		...
		print'</form>';
		print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&action=detail&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
		print '</table>';
	...
	if($action == 'edit_bareme'){
		...
		print '<div >';
		print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_prime='.$id_prime.'&rowid_id='.$rowid.'&id_convention='.$id_convention.'&id_societe='.$id_societe.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		...
		print '<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
		print'</form>';
		print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&action=liste_condition&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
		print '</table>';
	...
	if($action == 'nouvelle_condition_form'){
		...
		$verif_obj = $db->fetch_object($verif_result);
				print '<div >';
				print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=societe&id_prime='.$id_prime.'&id_convention='.$id_convention.'&id_societe='.$id_societe.'">';
				print '<input type="hidden" name="token" value="'.newToken().'">';
	...
	if($action == "liste_condition"){
		print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle condition", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=societe&action=nouvelle_condition_form&id_societe='.$id_societe.'&id_convention='.$id_convention.'&id_prime='.$id_prime , '', 1), '', 0, 0, 0, 1);
		$id = $id_prime;
		$head = Prime_indm_societe_Head($id, 2, $id_convention, $id_societe);
		print dol_get_fiche_head($head, 'information', "", -1, '');
		...
		//------------------------------------------------------------------------------------------------------------------------------------------
			...
			//------------------------------------------------------------------------------------------------------------------------------------------
					...
					print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'&id_societe='.$id_societe.'&id_convention='.$id_convention.'">'.img_delete('', '').'&nbsp;</a>';
					print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&id_societe='.$id_societe.'&id_convention='.$id_convention.'&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'">'.img_edit('', '').'</a></td>';
					print "<tr>";
					$numero ++;
					$i ++;
			}
			if($num == 0) print "<tr><td align='center' colspan='8'>Aucne condition disponible concernant cette Convention</td></tr>";
