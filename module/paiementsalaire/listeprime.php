...
$message = "";
$action = 'afficher';
$id_conv = GETPOST('id_convention', 'int');
	...
	if($action == "add_prime"){
		if(empty(GETPOST('libelle'))) {
			...
			if(empty($message)){
				...
				$variable_prime = GETPOST('variable_prime', 'int');
				...
				$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'primes (libelle, type_prime, commentaire, appliquee, exonere, affiche_bulletin, soumis_impot, soumis_cotisation, fk_convention, fk_societe, fk_accord_etablissement, active) 
				VALUES ("'.$libelle.'","'.$type_i.'","'.$desc_i.'",'.$applique.',"'.$exonere.'","'.$affiche_bulletin.'","'.$soumis_impot.'","'.$soumis_cotisation.'",0,0,0,0)';
				$result = $db->query($sql);
				if($result){
...
if($action == 'activate'){
	$id_prime = GETPOST('id_prime');
	if($id_prime == 1)
		$sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."condition_anciennete";
	else $sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."condition_prime WHERE fk_prime=".$id_prime;
...
print '<hr>';
	...
	print '<div >';
	print '<form name="add_prime"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime">';
	print '<input type="hidden" name="token" value="'.newToken().'">';
	print '<input type="hidden" name="action" value="add_prime">';
	print '<table>';
	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Libellé</label></td><td style="padding-top: 10px" id="libelle" ><input style="width: 500px" name ="libelle" value="'.GETPOST("libelle").'" size="30" type="text" /></td>';

	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Type</label></td><td style="padding-top: 10px">';
	...
	print '<tr><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Commentaire</label></td><td style="padding-top: 10px"><textarea style="width: 550px; height: 50px" name="commentaire" wrap="soft" cols="50" rows="3">'.GETPOST("commentaire").'</textarea>
	</td>';
	print "<tr ><td style='width: 200px; padding-top: 10px; padding-right: 30px' class=''><label>S'applique au </label></td><td style='padding-top: 10px'>";
	...
	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Retiré du Sal. Base</label></td><td style="padding-top: 10px">';
	...
	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Afficher sur bulletin de paye</label></td><td style="padding-top: 10px">';
	...
	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Soumis aux Impôts</label></td><td style="padding-top: 10px">';
	...
	print '<tr ><td style="width: 200px; padding-top: 10px; padding-right: 30px" class=""><label>Soumis à Cotisation</label></td><td style="padding-top: 10px">';
	...
	print '
		<div style="text-align: center; align-items: center; justify-content: center">
			<tr><td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>
			</form>
			<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&action=afficher" class="button">Annuler</a></td></tr>
		</div>
		';
...
if($action == 'afficher'){
	print load_fiche_titre($langs->trans("La liste des Primes"), '', '');
	
print '<hr>';
//table des champs et labels
print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une Prime exceptionnele", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=prime&action=create' , '', 1), '', 0, 0, 0, 1);
print '<table class="tagtable liste">';
print '<tr class="liste_titre"><td class="liste_titre" style="color: darkblue; padding: ; width : 5%;" >Nom</td>
<td class="liste_titre" style="color: darkblue; padding: ; width : 5%;" >Type</td>
<td class="liste_titre" style="color: darkblue; padding: ; width : 5%;" >Applique au</td>
<td class="liste_titre" style="color: darkblue; padding: ; width : 5%;" >Opération</td></tr>';
$indSql = "SELECT * FROM ".MAIN_DB_PREFIX."primes WHERE fk_convention=0 AND fk_societe=0 AND fk_accord_etablissement=0 ORDER BY active DESC";
	...
	if($result){
		...
		while ($i < $num){
			...
			if($obj->active == 1)
				print '<tr class="pair"><b>'.affiche_long_texte(img_picto("", "statut7", "class='paddingright pictofixedwidth'"), $obj->libelle, 0, './detail.php?idmenu=4399&mainmenu=paiementsalaire&leftmenu=prime&action=detailprime&id_prime='.$obj->rowid, 'nom', '', '', '', '').'<b></td>';
			else	
				print '<tr class="pair"><b>'.affiche_long_texte(img_picto("", "statut7_red", "class='paddingright pictofixedwidth'"), $obj->libelle, 0, '', 'nom', '', '', '', '').'<b>';
			...
			print '<td>';																			
			print'<a class="reposition" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=prime&action='.$acts[$obj->active].'&id_prime='.$obj->rowid.'&token='.newToken().'">'.$actl[$obj->active].'</a>&nbsp;';
			print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=prime&action=detail&id_prime='.$obj->rowid.'&id_convention='.$obj->fk_convention.'">'.img_edit().'</a>';	
			if($obj->rowid > 1)
				print '&nbsp;&nbsp;&nbsp;<a class="reposition editfielda" id="delete'.$i.'" onclick="myFunction('.$i.')" href="'.$url.'?mainmenu=paiementsalaire&leftmenu=prime&action=supprimer&id_prime='.$obj->rowid.'">'.img_delete().'</a>&nbsp;';
			print "</td></tr>";
			$i ++;
		}
        ...
        function myFunction(e){
           ...
           if(!confirm('Click sur OK pour confirmer cette suppression')){
               var lien = '".$_SERVER["PHP_SELF"]."?mainmenu=paiementsalaire&leftmenu=prime&action=afficher';
...
if($action == "saveedit_prime"){
	...
	if($action == 'detail'){
		...
		$id = $id_prime;
		$head = Prime_indm_Head($id, 2, $id_conv);
		...
		if($obj->fk_accord_etablissement != 0){
			$sql_accord_etablissement = "SELECT nom FROM ".MAIN_DB_PREFIX."accord_etablissement WHERE rowid=".($obj->fk_accord_etablissement);
		...
		print '<tr ><td style="width: 250px; padding-top: 10px;" class="fieldrequired"><a class="button" href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$obj->rowid.'&action=edit_prime">Modifier</a></td></tr>';
		print "</table>";
}
...
if($action == 'edit_prime'){
	...
	print '<div >';
	print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$obj->rowid.'">';
	print '<input type="hidden" name="token" value="'.newToken().'">';
	print '<input type="hidden" name="action" value="saveedit_prime">';
	print '<table>';
	print '<tr ><td style="width: 200px; padding-top: 10px;" class=""><label>Libellé</label></td><td style="width: 150px; padding-top: 10px" id="nom" ><input style="width: 500px; margin-bottom: 10px" type="text" name="libelle" value="'.$obj->libelle.'"/></td>';

	print '<tr ><td style="width: 250px; padding-top: 10px;" class=""><label>Type</label></td><td style="width: 150px; padding-top: 10px">';
	print '';
	$type_pr = "";
	if($obj->type_prime == "obligatoire"){				
	...
	print $type_pr.'</td></tr>';
	print '<tr ><td style="width: 250px; padding-top: 10px;" class=""><label>Commentaire</label></td><td style="width: 150px; padding-top: 10px"><textarea style="width: 550px; height: 50px; margin-bottom: 10px" name="commentaire" wrap="soft" cols="50" rows="3">'.$obj->commentaire.'</textarea>
	</td></tr>';
	print '<tr ><td style="width: 250px; padding-top: 10px;" class=""><label>S\'applique au</label></td><td style="width: 150px; padding-top: 10px">';
	print '<select style="width: 500px; margin-bottom: 10px" name="applique" id="applique" >';
	if($obj->appliquee == 1){
	...
	print'</td></tr>';
	if($obj->exonere == "oui"){
		print '<tr ><td style="padding-top: 10px; padding-right: 30px" class=""><label>Retiré du Sal. Base</label></td><td style="padding-top: 10px">';
		print '<input type="radio" value="oui" name="exonere" id="oui" checked><label for="oui" >Oui&ensp;</label>
	...
	print '<tr ><td style="padding-top: 10px; padding-right: 30px" class=""><label>Afficher sur bulletin de paye</label></td><td style="padding-top: 10px">';
	print '<select style="width: 500px" name="affiche_bulletin" id="affiche_bulletin" >';
	...
	print '<tr ><td style="padding-top: 10px; padding-right: 30px" class=""><label>Soumis aux Impôts</label></td><td style="padding-top: 10px">';
	print '<select style="width: 500px" name="soumis_impot" id="soumis_impot" >';
	...
	print '<tr ><td style="padding-top: 10px; padding-right: 30px" class=""><label>Soumis à Cotisation</label></td><td style="padding-top: 10px">';
	print '<select style="width: 500px" name="soumis_cotisation" id="soumis_cotisation" >';
	...
	<select style="width: 500px" name="convention" disabled>';
	...
	print '
		<div style="text-align: center; align-items: center; justify-content: center">
			...
			<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&action=detail&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>
		</div>
		';

}
if($action == "ajout_bareme"){
	if(empty(GETPOST("anciennete"))){
	...
	if(empty($message)){
		...
		$nombre_annee = GETPOST("nombre_annee");
		$taux = GETPOST("taux");
		if($taux != "" && $nombre_annee !=""){
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."condition_anciennete (fk_prime,fk_convention, nombre_annee, taux) VALUES (".$id_prime.",".$id_conv.",'".$nombre_annee."',".$taux.")";
			$result2 = $db->query($sql);
			$action = "liste_condition";
...
if($action == "saveedit_bareme"){
	...
	//-------------------------------
		...
		$rowid_id = GETPOST("rowid_id","int");
		$nombre_annee = GETPOST("nombre_annee","int");
		$taux = GETPOST("taux", "int");
		if($taux != "" && $nombre_annee !=""){
			$sql = "UPDATE ".MAIN_DB_PREFIX."condition_anciennete SET nombre_annee='".$nombre_annee."', taux=".$taux." WHERE rowid=".$rowid_id;
			$result2 = $db->query($sql);
			$message = "Barême Modifié ave succès";
			$action = "liste_condition";
		}else {
			$message = "Un problème est survenu";
			$action = "liste_condition";
...
if($action == 'edit_bareme'){
	...
	$anciennete_array = array("Anciennete","anciennete", "Ancinneté", "ancinneté", "anciennété", "Anciennété", "Anciennéte", "anciennéte", "Ancienété", "ancienété", "Ancieneté", "ancieneté", "Ancienete", "ancienete", "Acienete", "acienete", "Acieneté", "acieneté", "Acieneté", "écieneté");

		if(($id_prime == 1) || in_array($verif_obj->libelle, $anciennete_array)){
			$cond_exist = "SELECT * FROM ".MAIN_DB_PREFIX."condition_anciennete WHERE rowid=".$rowid;
			$result_exist = $db->query($cond_exist);//= $db->query($cond_exist);
			$exist_obj = $db->fetch_object($result_exist);

			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$id_prime.'&rowid_id='.$exist_obj->rowid.'&id_convention='.$id_conv.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			print '<input type="hidden" name="action" value="saveedit_bareme">';
			print '<input type="hidden" name="anciennete" value="anciennete">';
			print "<table>";
			print "<tr clas='pair' >";
			print "<td align='center' ><label>Nombre d'année(s)</label></td>";
			print "<td align='center' ><label>Taux(%)</label></td>";
			print "<td align='center' ></td>";
			print "</tr>";

			print '<tr clas="impair" >';
			print "<td><input type='text' name='nombre_annee' min='0' value='".$exist_obj->nombre_annee."' ></td>";
			print "<td><input type='text' name='taux' value='".$exist_obj->taux."' ></td>";
			//print '<tr><td ><br></td>';
			print '<td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&action=liste_condition&id_convention='.$id_conv.'&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		...
		print '<div >';
		print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$id_prime.'&rowid_id='.$rowid.'&id_convention='.$id_conv.'">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		print '<input type="hidden" name="action" value="saveedit_bareme">';
		print '<table>';
	...
	print'</form>';
	print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&action=liste_condition&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
	print '</table>';
	}
...
if($action == 'supprimer_anciennete'){
	$rowid = GETPOST("rowid_id", "int");

	$sql_delete = "DELETE FROM ".MAIN_DB_PREFIX."condition_anciennete WHERE rowid=".$rowid;
	$result = $db->query($sql_delete);
	if($result)
		$message = "Condition supprimée avec succès";
	$action = "liste_condition";
...
if($action == 'nouvelle_condition_form'){
	...
	$anciennete_array = array("Anciennete","anciennete", "Ancinneté", "ancinneté", "anciennété", "Anciennété", "Anciennéte", "anciennéte", "Ancienété", "ancienété", "Ancieneté", "ancieneté", "Ancienete", "ancienete", "Acienete", "acienete", "Acieneté", "acieneté", "Acieneté", "écieneté");
		...
		if($id_prime == 1 || in_array($verif_obj->libelle, $anciennete_array)){
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$id_prime.'&id_convention='.$id_conv.'">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
			...
			print '<tr clas="impair" >';
			print "<td><input type='text' name='nombre_annee' min='0' value='0' >";
			print info_admin($langs->trans('Mettez le symbole + pour indiquer toutes valeurs inférieur à la dernière ligne du barème'), 1)."</td>";
			print "<td><input type='number' name='taux' min='0' value='0' ></td>";
			print '<td style=" padding-right: 30px; padding-bottom: 30px"><td style=" padding-bottom: 30px"><input class="button" type="submit" value="Enregistrer" name=""/>';
			print'</form>';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=indemnite&action=liste_condition&id_convention='.$id_conv.'&id_prime='.$id_prime.'" class="button">Annuler</a></td></tr>';
			print '</table>';
		}else if($id_prime != 1){
			print '<div >';
			print '<form name="add"  method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_prime='.$id_prime.'&id_convention='.$id_conv.'">';
...
if($action == "liste_condition"){
	...
	print_barre_liste("", $page, $_SERVER["PHP_SELF"], "", "", "", "", "", "", 'bill', 0, dolGetButtonTitle("Créer une nouvelle condition", '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?idmenu=4399&mainmenu=paiementsalaire&leftmenu=prime&action=nouvelle_condition_form&id_convention='.$id_conv.'&id_prime='.$id_prime , '', 1), '', 0, 0, 0, 1);
	$id = $id_prime;
	$head = Prime_indm_Head($id, 2, $id_conv);
	print dol_get_fiche_head($head, 'information', "", -1, '');
	...
	$anciennete_array = array("Anciennete","anciennete", "Ancinneté", "ancinneté", "anciennété", "Anciennété", "Anciennéte", "anciennéte", "Ancienété", "ancienété", "Ancieneté", "ancieneté", "Ancienete", "ancienete", "Acienete", "acienete", "Acieneté", "acieneté", "Acieneté", "écieneté");
		...
		if($id_prime == 1 || in_array($verif_obj->libelle, $anciennete_array)){
			$nom_conv = "";
			print "<form>";
			print "<b>Convention : </b><select style='width:91%; margin-top: 20px' name='id_convention' id='id_convention' >";
			$convSql = "SELECT * FROM ".MAIN_DB_PREFIX."convention"; 
			$res_conv = $db->query($convSql);
			if($res_conv)
				$num_conv =  $db->num_rows($res_conv);
			    $a = 0;
				while ($a < $num_conv) {
					$conv_obj = $db->fetch_object($res_conv);
					if($a==0){
						if($id_conv==0){
							$id_conv = $conv_obj->rowid;
							print "<option value='".$conv_obj->rowid."' selected >".$conv_obj->nom."</option>";
							$nom_conv = $conv_obj->nom;
						}else if($id_conv == $conv_obj->rowid){
									print "<option value='".$conv_obj->rowid."' selected >".$conv_obj->nom."</option>";
								$nom_conv = $conv_obj->nom;
						}else print "<option value='".$conv_obj->rowid."' >".$conv_obj->nom."</option>";
					}else{
						if($id_conv == $conv_obj->rowid){
							print "<option value='".$conv_obj->rowid."' selected >".$conv_obj->nom."</option>";
							$nom_conv = $conv_obj->nom;
						}else print "<option value='".$conv_obj->rowid."' >".$conv_obj->nom."</option>";
				...
				var convention = document.getElementById("id_convention");
				convention.addEventListener("change", function () {
					var id_convention = convention.value;
					window.location.href = "'.$_SERVER["PHP_SELF"].'?mainmenu=paiementsalaire&leftmenu=prime&id_convention="+id_convention+"&id_prime='.$id_prime.'&action=liste_condition";
				},
				false,
				);
				</script>';
			...
			print "</select>";
			print "</form>";
			print "<table class='tagtable liste' style='margin-top: 20px'>";
			print "<tr class='pair'>";
			print "<td align='center' style='width: 130px; padding-left:20px; padding-right:20px; background-color: #6f89bd; color: white'>Nombre d'années</td>";
			print "<td align='center' style='width: 130px; padding-left:20px; padding-right:20px; background-color: #6f89bd; color: white'>Taux(%)</td>";
			print "<td align='center' style='width: 130px; padding-left:20px; padding-right:20px; background-color: #6f89bd; color: white'>Opérations</td>";
			print "<tr>";

			$cond_anc = "SELECT * FROM ".MAIN_DB_PREFIX."condition_anciennete WHERE fk_convention=".$id_conv;
			$result_anc = $db->query($cond_anc);//= $db->query($cond_ind);
			$num = $db->num_rows($result_anc);
			$i = 0;
			$last = 0;
			if($result_anc)
			while ($i < $num){
				$inf = "";
				$cond =  $db->fetch_object($result_anc);
					print "<tr class='impair'>";
					print "<td align='center' style='width: 150px; padding:10px;'>".$cond->nombre_annee;
					if($cond->nombre_annee == '+' || $cond->nombre_annee == '++' || $cond->nombre_annee == 'plus'){
						$var = 'Pour toutes Valeurs supérieur à '.$last;
						print info_admin($langs->trans($var), 1);
					}else $last = $cond->nombre_annee;
					print "</td>";
					print "<td align='center' style='width: 150px; padding:10px;'>".$cond->taux."</td>";
					print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=supprimer_anciennete&rowid_id='.$cond->rowid.'&id_prime='.$id_prime.'">'.img_delete('', '').'&nbsp;</a>';								
					print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&rowid_id='.$cond->rowid.'&id_prime='.$id_prime.'&id_convention='.$id_conv.'">'.img_edit('', '').'</a></td>';								
					print "</tr>";
			...
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>S'applique au</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Type Salarié</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Catégorie</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Conditions</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Min Peçu</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Montant</td>";
			print "<td align='center' style='width: 40px; padding-left:20px; padding-right:20px; color: white; background-color: #6f89bd'>Operation</td>";
			print "<tr>";
	...
	//------------------------------------------------------------------------------------------------------------------------------------------
		...
		//------------------------------------------------------------------------------------------------------------------------------------------
				...
				print '<td align="center"><a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=supprimer_bareme&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'">'.img_delete('', '').'&nbsp;</a>';								
				print '<a class="reposition editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_bareme&rowid_id='.$obj_ind->rowid.'&id_prime='.$obj->rowid.'">'.img_edit('', '').'</a></td>';								
				print "<tr>";
				$numero ++;
				$i ++;
