...
if($user->rights->paiementsalaire->societe->read){ 
    ...
    if(!empty($id_convention)){
        ...
        if($trouve == true){
          ...
          $array[] = array('label'=> 'Type de Complément','type'=> 'select', 'size'=>'', 'morecss'=>'', 'moreattr'=>'selected', 'name'=>'modele_bonus','values' => array_combine($key,$val));
          ...
          if($action == "supprimer_complement"){
            $mois = GETPOST("mois", "int");
              ...
              $sql_verif = "SELECT nom, prenom, libelle, nom_bonus, salaire_brut FROM ".MAIN_DB_PREFIX."bulletin_bonus WHERE annee=".$annee." AND mois=".$mois." AND fk_salarie=".$fk_salarie." AND fk_societe=".$id_societe;
              $res_verif = $db->query($sql_verif);
              $obj_verif = $db->fetch_object($res_verif);
              
              $array[] = array('label'=> 'Nom','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'disabled', 'name'=>'nom_prenom','value' => $obj_verif->prenom." ".$obj_verif->nom);
              $array[] = array('label'=> 'Libelle','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'disabled', 'name'=>'libelle','value' => $obj_verif->libelle);
              $array[] = array('label'=> 'Montant','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'disabled', 'name'=>'montant','value' => $obj_verif->salaire_brut);

                $url = $_SERVER['PHP_SELF']."?mainmenu=paiementsalaire&leftmenu=societe&id_societe=".$id_societe."&id_convention=".$id_convention."&annee=".$annee."&mois=".$mois."&fk_salarie=".$fk_salarie;
                $titre = 'Veillez confirmer la suppression de ce complément salaire';

                  $formconfirm = $monform->formconfirm(
                      $url, 
                      $titre, 
                      "", 
                      'supprimer_ok_complement', 
                      $array, 
                      '', 
                      1,
                      300,
          ...
          //La suppression après la confirmation
          if($action == "supprimer_ok_complement"){
              $mois = GETPOST("mois", "int");
              $annee = GETPOST("annee", "int");
              $id_societe = GETPOST("id_societe", "int");
              $fk_salarie = GETPOST("fk_salarie", "int");
              ...
              //Recupération des données à modifier
              $sql_verif = "SELECT nom, prenom FROM ".MAIN_DB_PREFIX."bulletin_bonus WHERE annee=".$annee." AND mois=".$mois." AND fk_salarie=".$fk_salarie." AND fk_societe=".$id_societe;
              $res_verif = $db->query($sql_verif);
              $obj_verif = $db->fetch_object($res_verif);

              //Recupération des données à modifier
              $sql_verif = "DELETE FROM ".MAIN_DB_PREFIX."bulletin_bonus WHERE annee=".$annee." AND mois=".$mois." AND fk_salarie=".$fk_salarie." AND fk_societe=".$id_societe;
              $res_verif = $db->query($sql_verif);
              if($res_verif){
                $message = "Un complément salaire supprimer avec succès";
                $sql_select = "SELECT firstname, lastname FROM ".MAIN_DB_PREFIX."user WHERE rowid=".$user->id;
                $obj = $db->fetch_object($db->query($sql_select));

                $action_effectue = 'Suppression du complément salaire du salarié '.$obj_verif->prenom.' '.$obj_verif->nom.' de '.$mois_tab[$mois-1]." ".$annee." de la société ".$obj_soc->nom;
          ...
          //Modfication du nom du complément salaire
          if($action == "modifier_nom_complement"){
            $mois = GETPOST("mois", "int");
              $annee = GETPOST("annee", "int");
              $id_societe = GETPOST("id_societe", "int");

              //Recupération des données à modifier
              $sql_verif = "SELECT nom_bonus FROM ".MAIN_DB_PREFIX."bulletin_bonus WHERE annee=".$annee." AND mois=".$mois." AND fk_societe=".$id_societe;
              $res_verif = $db->query($sql_verif);
              $obj_verif = $db->fetch_object($res_verif);
                
              $array[] = array('label'=> 'Nom du complement salaire','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'', 'name'=>'nom_bonus','value' => $obj_verif->nom_bonus);

                $url = $_SERVER['PHP_SELF']."?mainmenu=paiementsalaire&leftmenu=societe&id_societe=".$id_societe."&id_convention=".$id_convention."&annee=".$annee."&mois=".$mois;
                $titre = "Modification du nom du complément salaire";

                  $formconfirm = $monform->formconfirm(
                      $url, 
                      $titre, 
                      "", 
                      'save_modification_nom', 
                      $array, 
                      '', 
                      1,
                      180,
          ...
          if($action == "save_modification_nom"){
              ...
              $res_verif = $db->query($sql_verif);
              if($res_verif){
                $message = "Nom du complément salaire modifié avec succès";

                $sql_select = "SELECT firstname, lastname FROM ".MAIN_DB_PREFIX."user WHERE rowid=".$user->id;
                $obj = $db->fetch_object($db->query($sql_select));

                $action_effectue = 'Modification du nom du complément de '.$mois_tab[$mois-1].' '.$annee.' de la société '.$obj_soc->nom;
                $sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
                $sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Complements salaires")';
                $db->query($sql_log);
              }
          ...
          //Modfication d'un complément salaire
          if($action == "modifier_complement"){
            $mois = GETPOST("mois", "int");
              $annee = GETPOST("annee", "int");
              $id_societe = GETPOST("id_societe", "int");
              $fk_salarie = GETPOST("fk_salarie", "int");
              ...
              $array[] = array('label'=> 'Libelle','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'', 'name'=>'libelle','value' => $obj_verif->libelle);
              $array[] = array('label'=> 'Montant','type'=> 'text', 'size'=>'', 'morecss'=>'', 'moreattr'=>'', 'name'=>'montant','value' => $obj_verif->salaire_brut);

                $url = $_SERVER['PHP_SELF']."?mainmenu=paiementsalaire&leftmenu=societe&id_societe=".$id_societe."&id_convention=".$id_convention."&annee=".$annee."&mois=".$mois."&fk_salarie=".$fk_salarie;
                $titre = $obj_verif->prenom." ".$obj_verif->nom." ==> complément : ".$obj_verif->nom_bonus;
          ...
          //Modification après la confirmation
          if($action == "save_modification"){
            ...
            $res_verif = $db->query($sql_verif);
            if($res_verif){
              $sql_verif = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee." AND mois=".$mois." AND fk_salarie=".$fk_salarie." AND fk_societe=".$id_societe;
              $res_verif = $db->query($sql_verif);
              $obj_verif = $db->fetch_object($res_verif);

              if($obj_verif){
                  $montant_fixe = GETPOST('montant','int');
                  ...
                  $montant_pourcentage = "100";
                          ...
                          VALUES("'.$id_bull.'", "'.$nom_bonus.'", "'.$libelle.'","'.$obj_verif->nom.'","'.$obj_verif->prenom.'",'.$obj_verif->fk_salarie.',"'.$obj_verif->matricule.'","'.$obj_verif->situation_familiale.'",'.$obj_verif->nombre_enfant.','.$obj_verif->nombre_enfant_hand.',
                      ...
                      if($res_bulletin){
                        $message = 'Un complément de salaire modifieé avec succès </br>';
                        $sql_select = "SELECT firstname, lastname FROM ".MAIN_DB_PREFIX."user WHERE rowid=".$user->id;
                        $obj = $db->fetch_object($db->query($sql_select));

                        $action_effectue = 'Modification du complément salaire du salarié '.$obj_verif->prenom.' '.$obj_verif->nom.' de '.$mois_tab[$mois-1].' '.$annee.' de la société '.$obj_soc->nom;
                        $sql_log = 'INSERT INTO '.MAIN_DB_PREFIX.'log (fk_user, nom, prenom, quand, action_effectue, object_concerne)';
                        $sql_log .= ' VALUES('.$user->id.', "'.$obj->lastname.'","'.$obj->firstname.'",now(),"'.$action_effectue.'","Complements salaires")';
                        $db->query($sql_log);
                      }
          ...
          //Nettoyage des bulletin Complément
          if($action == "supprimer"){
            $mois = GETPOST("mois", "int");
              $annee = GETPOST("annee", "int");
              $id_societe = GETPOST("id_societe", "int");
...
//Enregistrement du Complément
        ...
        if($action == 'save_specialbonus'){
              ...
              $sql_verif = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee." AND mois=".$mois." AND fk_societe=".$id_societe;
                    ...
                    if($res_verif){
                      ...
                      while ($a < $num) {
                        $obj_verif = $db->fetch_object($res_verif);
                        $name = 'salarie'.$obj_verif->fk_salarie;
                        $libelle = 'libelle'.$obj_verif->fk_salarie;

                        $fk_sal = explode('salarie', $name)[1]; //explode('salarie', $name)[0] = 'salarie'
                        $label_sal = explode('libelle', $libelle)[1]; //explode('salarie', $name)[0] = 'libelle'
                          //Calcu et Enregistrement dans le bulletin Bonus                 
                        if(!empty(GETPOST($name,'int')) && !empty(GETPOST($libelle,'alpha'))){
                          //Calcul
                          $salaire_brut = 0;
                            $salaire_brut_imposable = 0;
                            $salaire_brut_cotisable = 0;
          ...
          if($action == "ajouterbonus"){
            ...
            if(!empty($message)){
              ...
              $titre = 'La configuration du Complément salaire';
                $url = $_SERVER['PHP_SELF']."?mainmenu=paiementsalaire&leftmenu=societe&id_societe=".$id_societe."&id_convention=".$id_convention."&annee=".$annee."&mois=".$mois;
          ...
          //formulaire d'ajout Bonus
             if($action == "ajoutbonus"){
                ...
                print "<table>";
                print "<tr><td>Libelle du bulletin : </td><td><input type='text' name='libelle' placeholder='Complément ".$mois_tab[($mois-1)]." ".$annee."' autofocus ></td></tr>";
                print "<tr><td>nom du Complément : </td><td><input type='text' name='nom_complement' placeholder='Bonus ".$mois_tab[($mois-1)]." ".$annee."' ></td></tr>";
                ...
                print "<table>";
                print "<tr class='liste_titre'><td>Libelle du Complément : </td><td><input type='text' name='libelle' placeholder='Complément ".$mois_tab[($mois-1)]." ".$annee."' autofocus ></td><td></td></tr>";
                ...
                $sql = "SELECT * FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee." AND mois=".$mois." AND fk_societe=".$id_societe;
                  ...
                  if($result){
                    ...
                    while ($a < $num) {
                      ...
                      $nom_p = $obj_sal->prenom." ".$obj_sal->nom;
                      $name = 'salarie'.$obj_sal->fk_salarie;
                      $libelle = 'libelle'.$obj_sal->fk_salarie;
                      if($a == 0){
                        print '<tr><td><label>'.$nom_p.'</label></td><td><input type="text" placeholder="Libelle" name="'.$libelle.'" value="'.GETPOST($libelle, "int").'"></td><td><input type="text" placeholder="Montant" name="'.$name.'" value="'.GETPOST($name, "int").'"></td></tr>';
                      }else print '<tr><td><label>'.$nom_p.'</label></td><td><input type="text" placeholder="Libelle" name="'.$libelle.'" value="'.GETPOST($libelle, "int").'"></td><td><input type="text" placeholder="Montant" name="'.$name.'" value="'.GETPOST($name, "int").'"></td></tr>';
        ...
        //$bouton = "";
					 ...
					 $bouton = "Voir pour tous les salariés";
           $url = "../doc/bulletin_bonus_tout_salarie.php?id_societe=".$id_societe."&id_convention=".$id_convention."&mois=".$mois_rech."&annee=".$annee_rech."&action=".$action;
...
*/
          $bulletin_pr_sql = "SELECT cloture FROM ".MAIN_DB_PREFIX."bulletin WHERE annee=".$annee_rech." AND mois=".$mois_rech." AND fk_societe=".$id_societe;
          ...
          if($num > 1)
					  print "<h2>Les Compléments de paie ".$mois_tab[$mois_rech-1]." ".$annee_rech." ==> <mark>".$object_liste[0]->nom_bonus.$modif."</mark><a target='_blank' href='".$url."' style='float: right;' class='button'>".$bouton."</a></h2>";
          else if ($num != 0) print "<h2>Les Compléments de paie ".$mois_tab[$mois_rech-1]." ".$annee_rech." ==> <mark>".$object_liste[0]->nom_bonus.$modif."</h2>";
					...
					if(count($object_liste) ==0){
						print "<tr><td colspan='3' align='center'><style='align:center;'>Aucun bulletin trouvé!</td></tr>";
					}
					print "</tbody>";
					print "</table>";
